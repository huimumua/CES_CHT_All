...
import com.askey.iotcontrol.bean.DeviceList;
import com.askey.iotcontrol.dao.ImportData;
import com.askey.iotcontrol.dao.ZwaveDevice;
import com.askey.iotcontrol.dao.ZwaveDeviceGroup;
import com.askey.iotcontrol.dao.ZwaveDeviceGroupManager;
...
import com.askey.iotcontrol.dao.ZwaveSceneManager;
import com.askey.iotcontrol.dao.ZwaveSchedule;
import com.askey.iotcontrol.dao.ZwaveScheduleManager;
import com.askey.iotcontrol.jni.ZwaveControlHelper;
import com.askey.iotcontrol.scheduler.ScheduleJobManager;
...
import static com.askey.iotcontrol.utils.Const.FILE_PATH;
import static com.askey.iotcontrol.utils.Const.SAVE_NODEINFO_FILE;
import static com.askey.iotcontrol.utils.Const.ZWCONTROL_CFG_PATH;
...
public class ZwaveControlService extends Service {
    ...
    private int flag;
    private static final String zwaveType = "Zwave";
    ...
    public void getRooms(){
        ...
        JSONObject jo = new JSONObject();
        JSONArray Jarray= new JSONArray();
        Log.e(LOG_TAG,"roomList.size() = "+roomList.size());
        try {
    ...
    public void setSwitchMultiLevel(String devType,int deviceId, int value, int duration){
        ...
        if (devType.equals(zwaveType)) {
            int res;
            if (value>99){
                res = ZwaveControlHelper.ZwController_SetSwitchMultiLevel(deviceId, 99, 1);
            }else{
                res = ZwaveControlHelper.ZwController_SetSwitchMultiLevel(deviceId, value, 1);
            }
            if (res == 0){
                result = "true";
            }
    ...
    public void setLampToColdWhite(String devType, int deviceId){
        ...
        if (devType.equals(zwaveType)){
            ...
            ZwaveControlHelper.ZwController_setSwitchColor(deviceId,0x02,0);
            ZwaveControlHelper.ZwController_setSwitchColor(deviceId,0x03,0);
            ZwaveControlHelper.ZwController_setSwitchColor(deviceId,0x04,0);
            JSONObject jsonResult = new JSONObject();
            try {
                jsonResult.put("Interface","setLampColor");
                jsonResult.put("NodeId",new Integer(deviceId));
    ...
    public void getLampColor(String devType, int deviceId){
        updateTimestamp(deviceId);
        if (devType.equals(zwaveType)){
            ZwaveControlHelper.ZwController_getSwitchColor(deviceId,0x00);
            ZwaveControlHelper.ZwController_getSwitchColor(deviceId,0x01);
    ...
    public String doOpenController() {
        Logg.i(LOG_TAG, "=====doOpenController=========");
        if (mCallBacks == null) {
            return null;
        }
    ...
    public void zwaveCallBack(byte[] result, int len) {
        ...
        */
            ...
            try {
                ...
                zwaveControlResultCallBack("getDeviceBattery", jsonObject.toString());
            } catch (JSONException e) {
                e.printStackTrace();
            }
            ...
            try {
                jsonObject = new JSONObject(jniResult);
                jsonObject.put("devType",zwaveType);
                zwaveControlResultCallBack("getSensorMultiLevel", jsonObject.toString());
            } catch (JSONException e) {
                e.printStackTrace();
            }
            ...
            try {
                jsonObject = new JSONObject(jniResult);
                jsonObject.put("devType",zwaveType);
                zwaveControlResultCallBack("getSensorNotification", jsonObject.toString());
            } catch (JSONException e) {
                e.printStackTrace();
            }
            ...
            try {
                jsonObject = new JSONObject(jniResult);
                jsonObject.put("devType",zwaveType);
                zwaveControlResultCallBack("GetSensorBinarySupportedSensor", jsonObject.toString());
            } catch (JSONException e) {
                e.printStackTrace();
            }
