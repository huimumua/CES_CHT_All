...
try_again:
  ...
  eeprom_read(EXT_FW_START_ADDR + ZW_FW_HEADER_LEN, (u8_t*) flash, sizeof(flash));

  if (zpg_program_chip(&pgmr, flash, sizeof(flash)) == 0)
  {
    pgmr.close();
    wait_1sec();
    /* Re-start the serial API process */
