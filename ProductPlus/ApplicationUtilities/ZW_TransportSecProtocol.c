...
ApplicationNodeInformation(
  ...
  BYTE *parmLength)
{
  CMD_CLASS_LIST* pCmdClassList = GetCommandClassList((0 != bTransportNID), SECURITY_KEY_NONE, 0);
...
uint8_t Transport_OnLearnCompleted(uint8_t nodeID)
{
  ...
  if ((nodeID == 0) || (nodeID == NODE_BROADCAST))    /* Was it reset, or did learn fail */
  {
    ZCB_timeout_kick();
  }
  else
  {
    if( NON_NULL( ZCB_pSetPowerDownTimeout ))
    {
      ZCB_pSetPowerDownTimeout(SEC_10_POWERDOWNTIMEOUT);
    }
  }
  return TRUE;
}
...
BOOL
TransportCmdClassSupported(  BYTE cmdClass, enum SECURITY_KEY eKey)
{
