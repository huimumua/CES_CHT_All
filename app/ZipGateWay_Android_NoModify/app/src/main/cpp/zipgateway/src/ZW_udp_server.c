    ...
    (*cbFunc)(BYTE, void* user),BOOL ackreq) //make CC_REENTRANT_ARG to place the local variables in the stack
{
  ...
  char udp_buffer[MAX_UDP_PAYLOAD_LEN + ZIP_HEADER_SIZE+5];

  if (datalen + ZIP_HEADER_SIZE + 5 > sizeof(udp_buffer))
  {
    PRINTF("ZW_SendData_UDP: Package is too large.\n");
    return;
  }
  ...
  pZipPacket->payload[0] = 5;
  pZipPacket->payload[1] = 0x80 | ENCAPSULATION_FORMAT_INFO;
  pZipPacket->payload[2] = 2;
  pZipPacket->payload[3] = EFI_SEC_LEVEL_NONE;
  pZipPacket->payload[4] = 0;
  ...
  memcpy(udp_buffer + ZIP_HEADER_SIZE+5, dataptr, datalen);
  ...
  udp_send_wrap(&client_conn, pZipPacket, ZIP_HEADER_SIZE + datalen +5);
}
void
...
void udp_send_wrap(struct uip_udp_conn* c, const void* buf, u16_t len) {
  ...
  if (is_4to6_addr((ip6addr_t*) &c->ripaddr)) {
    ...
    uint8_t node = nodeOfIP(&c->sipaddr);
    ...
    (*cbFunc)(BYTE, void* user))
{
  ...
  if (rnode)
  {
    ...
    if(!ZW_SendDataAppl(&p, dataptr, datalen, cbFunc, 0) && cbFunc != NULL) {
      cbFunc(TRANSMIT_COMPLETE_FAIL,0);
    }
  }
  else
  {
    ZW_SendData_UDP(c, dataptr, datalen, cbFunc,FALSE);
  }
}
...
static void ack_session_timeout(void* data) {
  ...
  if(s->cb) {
    s->cb(TRANSMIT_COMPLETE_FAIL,s->user);
  }
