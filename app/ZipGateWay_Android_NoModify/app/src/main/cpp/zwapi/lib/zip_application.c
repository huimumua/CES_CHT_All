...
#define WHITELIST_SET           0x17
#define WHITELIST_GET           0x18
#define WHITELIST_REPORT        0x19
#define WHITELIST_LIST_GET      0x15
#define WHITELIST_LIST_REPORT   0x16
#define WHITELIST_STATUS        0x1A
...
/**
zip_wl_list_parse - Parse DSK list
@param[in]	cmd_buf     Command buffer
@param[in]	cmd_len     Command length
@param[out]	dsk_list	DSK list
@post       Caller must free dsk_list if the call is successful.
@return Non-zero on success; else return zero
*/
static int zip_wl_list_parse(uint8_t *cmd_buf, uint16_t cmd_len, zw_wl_dsk_lst_t **dsk_list)
{
    uint8_t         dsk_cnt;
    uint8_t         dsk_len;
    zw_wl_dsk_lst_t *dsk_lst;
    int             i = 0;
    int             j = 0;
    ...
    dsk_lst = (zw_wl_dsk_lst_t *)calloc(1, sizeof(zw_wl_dsk_lst_t) + dsk_cnt * sizeof(zw_dsk_t));
    ...
    dsk_lst->dsk_cnt = dsk_cnt;

    while (cmd_len && (j < dsk_cnt))
    {
        dsk_len = cmd_buf[i++];
        cmd_len--;
        ...
        if (cmd_len >= dsk_len)
        {
            ...
            dsk_lst->dsk_lst[j].dsk_len = dsk_len;
            memcpy(dsk_lst->dsk_lst[j++].dsk, cmd_buf + i, dsk_len);
            i += dsk_len;
            cmd_len -= dsk_len;
        }
        else
...
/**
zip_wl_info_parse - Parse whitelist entry info
@param[in]	cmd_buf     Command buffer
@param[in]	cmd_len     Command length
@param[out]	info	    Whitelist entry info
@param[out]	info_cnt	Number of info in 'info'
@post       Caller must free info if the call is successful.
@return Non-zero on success; else return zero
*/
static int zip_wl_info_parse(uint8_t *cmd_buf, uint16_t cmd_len, wl_info_t **info, uint8_t *info_cnt)
{
    uint8_t         cnt = 0;
    uint8_t         meta_len;
    uint8_t         meta_type;
    ...
    wl_info = (wl_info_t *)calloc(cnt, sizeof(wl_info_t));
    ...
    while (cmd_len && (j < cnt))
    {
        ...
        switch (meta_type & 0x7F)
        {
            case WL_INFO_TYPE_NAME:
                wl_info[j].type = WL_INFO_TYPE_NAME;
                memcpy(wl_info[j++].info.name, cmd_buf + i, meta_len);
                break;

            case WL_INFO_TYPE_LOC:
                wl_info[j].type = WL_INFO_TYPE_LOC;
                memcpy(wl_info[j++].info.loc, cmd_buf + i, meta_len);
                break;
    ...
    switch (cmd_buf[1])
    {
        ...
        case WHITELIST_LIST_REPORT:
            if (cmd_len >= 4)
            {
                ...
                plt_mtx_lck(appl_ctx->zip_mtx);
                //Check for sequence number
                cb = (cmd_buf[2] == appl_ctx->nm_cb_info[NM_OP_WL_LIST_GET].seq_num)? appl_ctx->nm_cb_info[NM_OP_WL_LIST_GET].cb : NULL;
                plt_mtx_ulck(appl_ctx->zip_mtx);
                ...
                if (cb)
                {
                    ...
                    if (zip_wl_list_parse(cmd_buf + 3, cmd_len - 3, &dsk_lst))
                    {
                        cb(appl_ctx, dsk_lst);
                        free(dsk_lst);
                    }
        ...
        case WHITELIST_REPORT:
            if (cmd_len >= 3)
            {
                ...
                plt_mtx_lck(appl_ctx->zip_mtx);
                //Check for sequence number
                cb = (cmd_buf[2] == appl_ctx->nm_cb_info[NM_OP_WL_GET].seq_num)? appl_ctx->nm_cb_info[NM_OP_WL_GET].cb : NULL;
                plt_mtx_ulck(appl_ctx->zip_mtx);
                ...
                if (cb)
                {
                    ...
                    if (cmd_len >= 6 + dsk.dsk_len)
                    {
                        memcpy(dsk.dsk, cmd_buf + 4, dsk.dsk_len);

                        grnt_keys = (cmd_buf[4 + dsk.dsk_len] & 0x01)? cmd_buf[5 + dsk.dsk_len] : 0;

                        if (zip_wl_info_parse(cmd_buf + 6 + dsk.dsk_len, cmd_len - 6 - dsk.dsk_len, &info, &info_cnt))
                        {
                            cb(appl_ctx, &dsk, grnt_keys, info, info_cnt);
                            free(info);
                        }
                    }
                }
                                       ...
                                       uint16_t encap_fmt)
{
    ...
    {
        if (appl_nw_mgmt_hdlr(appl_ctx, src_addr, buf, len) == 0)
...
@return  0 on success, negative error number on failure
*/
int    zw_sec2_key_set(appl_layer_ctx_t *appl_ctx, uint8_t granted_key, uint8_t grant_csa)
{
    int             result;
    appl_snd_data_t snd_prm = {0};
...
/**
zip_wl_info_len_get - Get whitelist additional info length
...
@return  whitelist info length
*/
static unsigned zip_wl_info_len_get(wl_info_t *info, uint8_t info_cnt)
{
    unsigned len = 0;
    unsigned i;
...
/**
zip_wl_info_fill - Fill whitelist additional info into buffer
@param[in]	info		Buffer that stores additional information of the device
@param[in]	info_cnt    Number of additional information stored in "info".
@param[out]	buf		    Output buffer
@return  number of bytes filled in output buffer
*/
static unsigned zip_wl_info_fill(wl_info_t *info, uint8_t info_cnt, uint8_t *buf)
{
    unsigned j = 0;
    unsigned i;
    unsigned str_len;
...
@param[in]	info_cnt    Number of additional information stored in "info".
@param[in]	flag        Flag to clear one or all entries, see WL_FLAG_XXX.
@return  0 on success, negative error number on failure
*/
int  zw_wl_set(appl_layer_ctx_t *appl_ctx, uint8_t *dsk, uint8_t dsk_len, uint8_t grant_keys,
               wl_info_t *info, uint8_t info_cnt, uint8_t flag)
{
    int             result;
    int             i = 0;
    appl_snd_data_t snd_prm = {0};
    ...
    cmd_len = (flag & WL_FLAG_CLR_ALL)? 4 : (flag & WL_FLAG_CLR)? (5 + dsk_len) : (7 + dsk_len +  zip_wl_info_len_get(info, info_cnt));
    ...
    cmd[i++] = COMMAND_CLASS_NETWORK_MANAGEMENT_INCLUSION;
    cmd[i++] = WHITELIST_SET;
    i++;   //Sequence number
    cmd[i++] = flag;

    if (flag & WL_FLAG_CLR_ALL)
    {
        goto l_WL_SEND_CMD;
    }
    ...
    cmd[i++] = (grant_keys)? 1 : 0; //Advanced joining
    cmd[i++] = grant_keys;

    if (info_cnt)
    {
        cmd_len = zip_wl_info_fill(info, info_cnt, cmd + i);
        i += cmd_len;
    }
...
/**
zw_wl_list_get - Get all whitelist entries through callback
@param[in]	appl_ctx	Context
@param[in]	cb          Callback to report whitelist DSK list
@return  0 on success, negative error number on failure
*/
int  zw_wl_list_get(appl_layer_ctx_t *appl_ctx, zw_wl_list_fn cb)
{
    int             result;
    appl_snd_data_t snd_prm = {0};
    uint8_t         cmd[] = {COMMAND_CLASS_NETWORK_MANAGEMENT_INCLUSION, WHITELIST_LIST_GET, 0};
    ...
    if (result == 0)
    {   //Save the callback function
        appl_ctx->nm_cb_info[NM_OP_WL_LIST_GET].cb = cb;
        appl_ctx->nm_cb_info[NM_OP_WL_LIST_GET].seq_num = cmd[2];
    }
    plt_mtx_ulck(appl_ctx->zip_mtx);
...
/**
zw_wl_get - Get whitelist entry through callback
@param[in]	appl_ctx	Context
@param[in]	dsk		    Device Specific Key (DSK)
@param[in]	dsk_len		Device Specific Key length
@param[in]	cb          Callback to report whitelist entry info
@return  0 on success, negative error number on failure
*/
int  zw_wl_get(appl_layer_ctx_t *appl_ctx, uint8_t *dsk, uint8_t dsk_len, zw_wl_info_fn cb)
{
    int             result;
    int             i = 0;
    appl_snd_data_t snd_prm = {0};
    uint8_t         cmd[4 + MAX_DSK_KEY_LEN];
    ...
    if (result == 0)
    {   //Save the callback function
        appl_ctx->nm_cb_info[NM_OP_WL_GET].cb = cb;
        appl_ctx->nm_cb_info[NM_OP_WL_GET].seq_num = cmd[2];
    }
    plt_mtx_ulck(appl_ctx->zip_mtx);
