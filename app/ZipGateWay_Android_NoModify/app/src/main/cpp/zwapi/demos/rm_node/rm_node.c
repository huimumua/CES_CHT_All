/**
@file   rm_node.c - Sample application to remove a node from Z-wave network.
...
int main(int argc, char **argv)
{
    ...
    while (1)
    {
        ...
        else if ((appl_ctx.rm_status == RM_NODE_STS_UNKNOWN) || (appl_ctx.rm_status == RM_NODE_STS_DONE))
        {
            switch (choice)
            {
                case '1':
            ...
            printf("\n(1) %s\n(x) Exit\n", (appl_ctx.rm_status == RM_NODE_STS_PROGRESS)?
                   "Abort remove node operation" : "Remove node");
            printf("Select your choice:\n");
        }
        else    //RM_NODE_STS_PROGRESS
        {
            switch (choice)
            {
                case '1':
                    ...
                    if (result == 0)
                    {
                        printf("Remove node operation aborted.\n");
                        appl_ctx.rm_status = RM_NODE_STS_UNKNOWN;
                    }
                    else
                    {
                        printf("Remove node operation can't be aborted, error:%d\n", result);
                    }
                    break;
