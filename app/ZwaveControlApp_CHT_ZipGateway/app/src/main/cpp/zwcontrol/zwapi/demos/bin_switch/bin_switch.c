/**
@file   bin_switch.c - Sample application to control a binary switch.
...
/**
bin_sw_intf_get - Search for the first binary switch interface
@param[in]  net         network handle
@param[out] bin_sw_if   first binary switch interface found
@return  0 on success; otherwise negative number
*/
static int bin_sw_intf_get(zwnet_p net, zwifd_t *bin_sw_if)
{
    int         result;
    zwnoded_t   node;
    zwepd_t     ep;
    ...
    while (1)
    {
        ...
        else
        {
            switch (choice)
            {
                case '1':
                case '2':
                    result = bin_sw_intf_get(appl_ctx.zwnet, &bin_sw_if);
                    if (result == 0)
                    {
                        result = zwif_switch_set(&bin_sw_if, (choice == '1')? 1:0, NULL, NULL);
                        if (result != 0)
                        {
                    ...
                    {
                        printf("Searching of binary switch interface with error:%d\n", result);
                    }
                    break;
