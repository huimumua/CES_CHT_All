...
static void frm_snd_tmout_cb(void *data)
{
...
#ifdef  RETRY_USING_NEW_DTLS_SESSION
    result = tpt_snd_req_create(&frm_ctx->tpt_ctx, sock_addr, NULL,
                                frm_ctx->resnd_frm_buf, frm_ctx->resnd_frm_len,
                                (frm_ctx->resnd_frm_wkup_poll)? TPT_SEND_FLG_USE_HOST_PORT : TPT_SEND_FLG_NEW_DTLS_SESS);
#else
    if (frm_ctx->frm_rcvd)
    ...
    else
    {
        result = tpt_snd_req_create(&frm_ctx->tpt_ctx, sock_addr, NULL,
                                    frm_ctx->resnd_frm_buf, frm_ctx->resnd_frm_len,
                                    (frm_ctx->resnd_frm_wkup_poll)? TPT_SEND_FLG_USE_HOST_PORT : 0);
    }
#endif
...
l_RPT_FRAME_ERROR:
    ...
    frm_ctx->snd_frm_sts_cb(frm_ctx, status, seq_num, NULL);
...
int frm_send(frm_layer_ctx_t *frm_ctx, frm_snd_prm_t *prm)
{
    ...
    if (prm->cmd_buf && (prm->cmd_len > 0))
    {
        ...
        //Check whether the content is header extension or z-wave command
        ...
        else
        {
            ...
            if (prm->flag & ZIP_FLAG_ENCAP_INFO)
            {
                ...
                opt_ptr = zip_frm + ZIP_FRM_HDR_SIZE;

                *opt_ptr++ = ZIP_FRM_ENCAP_FMT_SZ; //Total option size
...
#ifndef  RETRY_USING_NEW_DTLS_SESSION
        frm_ctx->frm_rcvd = 0;
#endif
...
@return  0 on success, negative error number on failure
*/
static int frm_keep_alive_resp_send(frm_layer_ctx_t *frm_ctx, struct sockaddr *dst_addr)
{
    ...
    result = tpt_snd_req_create(&frm_ctx->tpt_ctx, dst_addr, NULL, zip_frm, ZIP_FRM_KEEP_ALIVE_SZ, 0);
...
@return             Pointer to the option; NULL if the option is not found or error
*/
...
/**
frm_hdr_ext_chk - Check frame header extension for unrecognized option
@param[in] buf      Buffer that stores the frame header extension
@param[in] dat_len  Number of bytes in buf
    ...
    if (buf[0] == COMMAND_CLASS_ZIP)
    {
        ...
        {   //ZIP packet
            ...
            else    //Check for response
            {
                ...
                //Check for the source address (could be Z/IP router address) and sequence number to match the
                ...
                {
                    frm_ctx->snd_frm_sts_cb(frm_ctx, snd_sts, buf[4], src_addr);
                }
            }
        ...
        {   //ZIP keep alive request
            ...
            plt_sleep(10);
    #endif
            frm_keep_alive_resp_send(frm_ctx, src_addr);
    #if defined(_WINDOWS) || defined(WIN32) || defined(OS_MAC_X)
            //Workaround for Windows Winsock and OSX sendto() return o.k. but doesn't send the packet
            plt_sleep(10);
    #endif
...
@param[in] frm_ctx      Context
@param[in] zip_router   Z/IP router address
...
@return  0 on success, negative error number on failure
*/
int frm_node_id_get(frm_layer_ctx_t *frm_ctx, struct sockaddr *zip_router, uint8_t *tgt_ip, int retry)
{
    ...
    result = tpt_snd_req_create(&frm_ctx->tpt_ctx, zip_router, NULL, zip_frm, ZIP_FRM_ND_SOLICIT_SZ,
...
@return  0 on success, negative error number on failure
*/
int frm_node_ipv6_get(frm_layer_ctx_t *frm_ctx, struct sockaddr *zip_router, uint8_t node_id)
{
    ...
    result = tpt_snd_req_create(&frm_ctx->tpt_ctx, zip_router, NULL, zip_frm, ZIP_FRM_ND_INV_SOLICIT_SZ, 0);
...
/**
frm_gw_send - Send frame to Z/IP router/gateway without Z/IP encapsulation
@param[in] frm_ctx      Context
