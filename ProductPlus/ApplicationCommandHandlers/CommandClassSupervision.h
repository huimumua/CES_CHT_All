 ...
 */
#define CommandClassSupervisionVersionGet() SUPERVISION_VERSION
...
typedef struct
{
  BYTE cmdClass; /**< Input */
  BYTE cmd; /**< Input */
  BYTE properties1; /**< Input: bit[0..5] session id, bit[7] status update of type cc_supervision_status_updates_t.
...
/**
 * @brief Handler for command class Supervision
 ...
 * 0xFF Reserved
 * @param[out] pCallback callback function pointer returning status on job. Can be initialiazed to NULL.
