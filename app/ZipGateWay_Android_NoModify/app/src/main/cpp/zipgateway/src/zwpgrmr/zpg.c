...
#define USB_READ_BURST 3
/*usb_loader_part1.hex*/
const u8_t usb_loader_part1[] =
{
  #include "usb_loader_part1"
};

const u8_t usb_loader_part2[] =
{
  #include "usb_loader_part2"
};
...
zpg_program_chip(zw_pgmr_t* p, u8_t* data, int size)
{
  ...
  if (p->apm && ! (p->usb == 2))
  {
    ...
    zpgp_set_lock_bits(p, RBAP, lockbits[RBAP] & ~RBAP_AUTOPROG0);
  }
