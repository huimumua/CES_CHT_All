...
public class WallMoteActionAdapter extends RecyclerView.Adapter<WallMoteActionAdapter.ViewHolder> implements View.OnClickListener{
    ...
    public void onBindViewHolder(WallMoteActionAdapter.ViewHolder holder, int position) {
        String type = wallMoteActionList.get(position).getType();
//        if ("BULB".equals(type)) {
//
...
//            holder.tvType.setText("Dimmer Control");
//        }
        int endpointId = Integer.parseInt((String) wallMoteActionList.get(position).getEndpointId());
        int groupId = Integer.parseInt((String) wallMoteActionList.get(position).getGroupId());
        switch (groupId) {
            case 2:
...
//        }
        ...
        holder.tvName.setText(wallMoteActionList.get(position).getName());     //需要通过wallMoteActionInfo获取设备名称
