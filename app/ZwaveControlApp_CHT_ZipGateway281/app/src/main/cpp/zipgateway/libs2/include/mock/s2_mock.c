...
uint8_t S2_send_data(struct S2* ctx, const s2_connection_t* dst ,const uint8_t* buf, uint16_t len)
{
  mock_t * p_mock;
...
/*
  ...
  MOCK_CALL_COMPARE_INPUT_UINT8_ARRAY(p_mock->compare_rule_arg[0], p_mock->expect_arg[0].p, sizeof(network_key_t),
                                                                   net_key,                 sizeof(network_key_t));*/
/*  MOCK_CALL_COMPARE_INPUT_UINT8_ARRAY(p_mock->compare_rule_arg[1], p_mock->expect_arg[1].p, p_mock->expect_arg[2].v,
...
#include <stdio.h>
uint8_t S2_network_key_update(struct S2 *ctx, security_class_t class_id,const network_key_t net_key, uint8_t temp_key_expand)
{
  mock_t * p_mock;
