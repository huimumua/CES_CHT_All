...
static void parse_prog_args()
{
  ...
  while ((opt = getopt(prog_argc, prog_argv, "mnc:s:f:e:t:")) != -1)
  {
    switch (opt)
    {
    case 'n':
    ...
    case 'x':
      ...
      break;
    default: /* '?' */
      fprintf(stderr, "Usage: %s [-c cfgfile] [-s serail_dev] [-f serial_log] [-e eeprom_file] [-t tun_script]\n",
          prog_argv[0]);
      exit(EXIT_FAILURE);
    }
  }
...
void ConfigInit()
{
  ...
  config_open();
  {
    ...
    cfg.ipv4disable = atoi(config_get_val("ZipIp4Disable", "0"));
