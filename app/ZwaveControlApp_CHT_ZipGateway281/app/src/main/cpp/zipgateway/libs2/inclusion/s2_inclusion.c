...
#ifdef ZW_CONTROLLER
#define TEMP_KEY_SECURE             3       //< Value identifying index for the temporary key in S2 network key when transmitting secure frames.
#define NETWORK_KEY_SECURE          4       //< Value identifying index for the current network key in S2 network key when transmitting secure frames.
#else
...
static const uint8_t m_key_slot_key_class_pair[] = {
  ...
  0x00,                         // TEMP_KEY_SECURE          is class id 3 = index 3 in array, which is not a valid value for network key exchange, hence 0x00.
  KEY_CLASS_S0,                 // S0 key                   is class id 5 = index 5 in array. Index only used temporary to exchange S0 key
};
...
//        ZW_DEBUG_SEND_WORD_NUM((uint16_t)s2_transition_table[i].action);
        ...
        switch (s2_transition_table[i].action)
        {
          ...
          case S2_ABORT_ACTION:
            inclusion_failed_evt_push(0x00);
          break;
...
static void s2_send_net_key_get(void)
{
  ...
  // If a transfer end is received then it must be ensured that the KEY was verified and that
  // including side has set KeyRequestComplete to '0'.
  else if ((SECURITY_2_TRANSFER_END == mp_context->buf[SECURITY_2_COMMAND_POS]) &&
           (SECURITY_2_KEY_VERIFIED != (mp_context->buf[SECURITY_2_TRANSFER_END_FLAGS_POS] & (SECURITY_2_KEY_REQ_COMPLETE | SECURITY_2_KEY_VERIFIED))))
  {
    process_event(S2_INCLUSION_ERROR);
    return;
  }
