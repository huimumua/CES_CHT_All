...
import com.askey.iotcontrol.net.TCPServer;
import com.askey.iotcontrol.net.UDPConnectin;
import com.askey.iotcontrol.net.awsIoTClient;
...
public class MQTTBroker extends Service {
    ...
    private Server localmqttServer = new Server();

    private TCPServer mTCPServer;                     // TCP server
    private ZwaveDeviceManager zwDevManager;          // for get/set device info from db
    public static ZwaveControlService zwaveService;
    ...
    public void onCreate() {
        ...
        Log.i("MQTTClient", "MQTT Local Server = [" + localmqttServer.getServerStatus() + "]");
    ...
    public void onDestroy() {
        ...
        if (localmqttServer.getServerStatus()) {
    ...
    public static void handleMqttIncomingMessage(String TopicName, String mqttMessage) throws JSONException {
        ...
        if (Const.remoteMqttFlag) {
            ...
            for (int idx = 0; idx < DeviceInfo.remoteSubTopiclist.size(); idx++) {
                if (!DeviceInfo.localSubTopiclist.contains(DeviceInfo.remoteSubTopiclist.get(idx))) {
                    unsubscribeTopic(DeviceInfo.remoteSubTopiclist.get(idx));
                }
            }
        }
    }
        ...
        public void zwaveControlResultCallBack(String className, String result) {
            ...
            if (className.equals("addDevice") || className.equals("removeDevice")) {
                ...
                try {
                    ...
                    if (result!=null && Utils.isGoodJson(result)) {
    ...
    private void reNameDevice(String result) {
        try {
            ...
            ZwaveDevice devNode = zwDevManager.queryZwaveDevices(Integer.valueOf(NodeId));

            if (devNode.getCategory().equals("BULB") ){
                name = devNode.getRoomName() + "light";
            } else{
                name = devNode.getRoomName() + devNode.getCategory();
            }

            Log.i(LOG_TAG, "devType=" + devType + " NodeId=" + NodeId + " roomName=" +devNode.getRoomName()+" devCate="+devNode.getCategory());

            if(name.contains(" ")) {
                name = name.replaceAll(" ", "");
            }

            Log.i(LOG_TAG,"name = " +name.toLowerCase());

            if (Const.awsIoTFlag) {
                awsIoTClient.createIotThing(Const.PublicTopicName + devType + NodeId, name.toLowerCase());
                awsIoTClient.subscribeAwsMQTT(Const.PublicTopicName + devType + NodeId);
            }
            publishMessage(Const.PublicTopicName + devType + NodeId, result);
        } catch (JSONException e) {
            e.printStackTrace();
                ...
                if (className.equals("addDevice")) {
                    ...
                    try {
                        ...
                        if (tNodeId.equals("fail")){
                            ...
                            if (Const.awsIoTFlag) {
                                awsIoTClient.deleteIotThing(Const.PublicTopicName + devType + tNodeId);
                            }
                            message.put("Result", "true");
                        }
                    } catch (JSONException e) {
