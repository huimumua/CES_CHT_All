...
#else
#error DEVELOPMENT or RELEASE compile flag not defined.
#endif
...
#define TB1_TIMEOUT  SECONDS_10    //< Timeout TB1 is used after neighbor discovery has completed and the time until KEX Get must be received from the including node.
#define TB2_TIMEOUT  SECONDS_10    //< Timeout TB2 is used after sending KEX Report and is the time until KEX Set must be received from the including node.
...
#define ACCESS_KEY_SLOT             2       //< Slot for access key. This value matches the ordering of security_key_t in ZW_security_api.h - do not change
...
#else
extern struct S2 the_context;
#define mp_context (&the_context)
#define MP_CTX_DEF
 ...
 * and S2_ACCESS in index 2. This defines the API between the S2 module and
 * the glue layer around it.
 * */
...
static void inclusion_failed_evt_push(void)
{
  ...
  // Post event upwards to inform that inclusion of node A has failed (Invalid scheme).
  p_s2_event->event_type        = S2_NODE_INCLUSION_FAILED_EVENT;
  p_s2_event->evt.s2_event.peer = mp_context->peer;
  p_s2_event->evt.s2_event.s2_data.inclusion_fail.kex_fail_type =
    mp_context->u.inclusion_buf[SECURITY_2_KEX_FAIL_FAIL_TYPE_POS];
...
static void inclusion_failed_frame_send(uint8_t error, uint8_t secure)
{
  ...
  {
    mp_context->peer.class_id = TEMP_KEY_SECURE;
    mp_context->peer.tx_options = 0;
    s2_inclusion_send_data();
  }
  else
  {
...
static uint8_t validate_echo_kex_report(void)
{
  if ((m_schemes == mp_context->buf[SECURITY_2_KEX_REP_SCHEME_POS]) &&
      ...
      ((mp_context->kex_report_byte2 | SECURITY_2_ECHO_ON) == mp_context->buf[2]) )
  {
    return 0;
  }

  inclusion_failed_frame_send(KEX_FAIL_AUTH, TEMP_KEY_SECURE);
...
void s2_inclusion_challenge_response(struct S2 *p_context, uint8_t include, const uint8_t* response)
{
  ...
  if (ACCEPT_INCLUSION == include)
  {
    ...
    if(mp_context->inclusion_mode == INCLUISON_MODE_SSA) {
      if (mp_context->key_granted & (KEY_CLASS_S2_AUTHENTICATED | KEY_CLASS_S2_ACCESS))
      {
        bytes_to_copy = 2;
      }
      else
    ...
    memcpy(mp_context->public_key, response, bytes_to_copy);
...
void s2_inclusion_key_grant(struct S2 *p_context, uint8_t include, uint8_t keys, uint8_t csa)
{
  ...
  if (ACCEPT_INCLUSION == include)
  {
    mp_context->inclusion_mode = csa ? INCLUISON_MODE_CSA : INCLUISON_MODE_SSA;
...
void s2_inclusion_including_start(struct S2 *p_context, s2_connection_t* conn)
{
  ...
  {
    mp_context->peer = *conn;
    mp_context->peer.tx_options = 0;
    mp_context->inclusion_mode = INCLUISON_MODE_SSA;
...
void s2_inclusion_post_event(struct S2 *p_context)
{
  ...
  if (crypt_rule != mp_context->peer.class_id)
  {
    if ((DUAL_RULE_SUPPORT != m_frame_rules[event_rule_index][ENCRYPTION_SUPPORT_INDEX])
        || (mp_context->peer.class_id != m_frame_rules[event_rule_index][ENCRYPTION_RULE_INDEX_B]))
    {
...
uint8_t s2_inclusion_init(uint8_t schemes, uint8_t curves, uint8_t keys)
{
  ...
  CHECK_AND_FAIL((~SECURITY_2_KEY_MASK & keys) ||
                 ((SECURITY_2_KEY_MASK & keys) == 0), KEX_FAIL_KEX_KEY);
...
static void s2_send_kex_report(void)
{
  ...
  mp_context->u.inclusion_buf[SECURITY_2_KEX_REP_ECHO_POS]   = SECURITY_2_ECHO_OFF;
  ...
  if(mp_context->inclusion_mode == INCLUISON_MODE_CSA) {
    mp_context->u.inclusion_buf[SECURITY_2_KEX_REP_ECHO_POS] |= SECURITY_2_CSA_ON;
  }
  mp_context->kex_report_byte2 = mp_context->u.inclusion_buf[2];
...
static void s2_send_pub_key_b(void)
{
  ...
  if ((0 == (~m_keys & support)) && (m_keys & support))
  {
  ...
  {
    if(mp_context->inclusion_mode!= INCLUISON_MODE_CSA) //We did not ask for this
    {
      support = KEX_FAIL_KEX_KEY;
      goto error_handling;
    }
  ...
  if(mp_context->inclusion_mode == INCLUISON_MODE_SSA) {
    if (mp_context->key_granted & ~(KEY_CLASS_S0 | KEY_CLASS_S2_UNAUTHENTICATED))
    {
...
static void s2_joining_start(void)
{
  ...
  /** When starting secure part of inclusion the node has already completed
   *  node id assignment in network. Therefore old keys are no longer valid
...
static void s2_send_echo_kex_set(void)
{
  ...
  if(mp_context->inclusion_mode == INCLUISON_MODE_CSA) {
    mp_context->u.inclusion_buf[SECURITY_2_KEX_SET_ECHO_POS]   =
        (mp_context->kex_set_byte2 & SECURITY_2_KEX_SET_RESERVED_MASK) | SECURITY_2_ECHO_ON |SECURITY_2_CSA_ON ;
  } else {
    mp_context->u.inclusion_buf[SECURITY_2_KEX_SET_ECHO_POS]   =
        (mp_context->kex_set_byte2 & SECURITY_2_KEX_SET_RESERVED_MASK) | SECURITY_2_ECHO_ON;
  }
  ...
  mp_context->peer.class_id = TEMP_KEY_SECURE;
  mp_context->peer.tx_options = S2_TXOPTION_VERIFY_DELIVERY;
  ...
  S2_send_data(mp_context, &mp_context->peer, mp_context->u.inclusion_buf, mp_context->inclusion_buf_length);
...
//  if(mp_context->length >= SECURITY_2_PUB_KEY_KEY_POS + sizeof(public_key_t)) {
    ...
    s2_event->evt.s2_event.peer = mp_context->peer;
...
static void s2_send_net_key_get(void)
{
  ...
  {
    // Error situation. We should never get into a situation where we don't have keys to exchange.
    // Internal error, therefore silently abort.
    return;
  }
...
static void s2_send_net_key_verify(void)
{
  uint8_t received_key = mp_context->buf[SECURITY_2_NET_KEY_REP_GRANT_KEY_POS];
  ...
  mp_context->peer.class_id = NETWORK_KEY_SECURE;
  mp_context->peer.tx_options = 0;
...
error_handling:
  inclusion_failed_frame_send(KEX_FAIL_KEY_REPORT, TEMP_KEY_SECURE);
  process_event(S2_INCLUSION_ERROR_SENT);
}
...
static void s2_send_final_transfer_end(void)
{
  ...
  ZW_DEBUG_SEND_STR("send_final_transfer_end\r\n");
  mp_context->peer.class_id = TEMP_KEY_SECURE;
  mp_context->peer.tx_options = 0;
...
static void s2_joining_complete(void)
{
  ...
  s2_restore_keys(mp_context);
  s2_event->event_type        = S2_NODE_JOINING_COMPLETE_EVENT;
  s2_event->evt.s2_event.peer = mp_context->peer;
...
static uint8_t validate_reported_keys(void)
{
  ...
  mp_context->key_granted  = (mp_context->buf[SECURITY_2_KEX_REP_KEYS_POS] & SECURITY_2_KEY_MASK);
...
static uint8_t validate_requested_key(void)
{
  ...
  if ((0 == (req_key & (req_key - 1))) &&
      (req_key & mp_context->key_granted))
  {
    return 0;
  }

  inclusion_failed_frame_send(KEX_FAIL_KEY_GET, TEMP_KEY_SECURE);
...
static uint8_t validate_echo_kex_set(void)
{
  ...
  if ((mp_context->scheme_support == mp_context->buf[SECURITY_2_KEX_SET_SCHEME_POS]) &&
      ...
      (mp_context->key_granted    == mp_context->buf[SECURITY_2_KEX_SET_KEYS_POS]) &&
      (
       (mp_context->inclusion_mode == INCLUISON_MODE_CSA) ==
           ((mp_context->buf[SECURITY_2_KEX_REP_ECHO_POS] & SECURITY_2_CSA_ON)>0) )
  )
  {
    return 0;
  }
...
static void s2_kex_rep_recv(void)
{
  ...
  mp_context->kex_report_byte2 = mp_context->buf[2];

  s2_event = (zwave_event_t *)m_event_buffer;
  s2_event->event_type                                    = S2_NODE_INCLUSION_KEX_REPORT_EVENT;
  s2_event->evt.s2_event.peer                             = mp_context->peer;
  s2_event->evt.s2_event.s2_data.kex_report.security_keys = mp_context->buf[SECURITY_2_KEX_REP_KEYS_POS];
...
static void s2_send_echo_kex_report(void)
{
  uint8_t ret_val = validate_echo_kex_set();
  if (ret_val != 0)
  {
    process_event(S2_INCLUSION_ERROR_SENT);
    return;
  ...
  mp_context->u.inclusion_buf[SECURITY_2_COMMAND_CLASS_POS]  = COMMAND_CLASS_SECURITY_2;
  mp_context->u.inclusion_buf[SECURITY_2_COMMAND_POS]        = KEX_REPORT;
  mp_context->u.inclusion_buf[SECURITY_2_KEX_REP_ECHO_POS]   = mp_context->kex_report_byte2 | SECURITY_2_ECHO_ON;
  ...
  mp_context->u.inclusion_buf[SECURITY_2_KEX_REP_KEYS_POS]   = mp_context->key_exchange;
  mp_context->inclusion_buf_length                           = SECURITY_2_KEX_REPORT_LENGTH;
...
static void s2_send_net_key_report(void)
{
  uint8_t ret_val = validate_requested_key();
  if (ret_val != 0)
  {
    process_event(S2_INCLUSION_ERROR_SENT);
    return;
  ...
  keystore_network_key_read(mp_context->u.inclusion_buf[SECURITY_2_NET_KEY_REP_GRANT_KEY_POS],
