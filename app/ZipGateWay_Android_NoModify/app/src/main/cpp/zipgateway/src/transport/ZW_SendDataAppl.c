...
void
ZW_SendDataAppl_CallbackEx(uint8_t status, void*user) REENTRANT
{
...
send_first()
{
  ...
  {
    ZW_SendDataAppl_CallbackEx(TRANSMIT_COMPLETE_ERROR, current_session);
  }
}
...
ZW_SendDataApplAbort(uint8_t handle ) {
  ...
  if(s == current_session) {
    ...
    if (s->callback)
    {
      s->callback(TRANSMIT_COMPLETE_FAIL, s->user);
    }
  }
}
