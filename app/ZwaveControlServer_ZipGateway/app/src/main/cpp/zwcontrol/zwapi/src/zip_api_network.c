...
#define CTLR_SM_GET_DSK ///< Enable get Z/IP gateway DSK in controller info state-machine
...
int           zwnet_sm_ver_rpt_hdlr(zwnet_p nw, zwif_p *ver_intf, void *ver_rpt_cb, uint16_t cmd_cls, uint8_t cmd_cls_ver,
                                    int tx_opt, int skip_extra_cmd_cls);
...
static void   zwnet_unsolic_wl_sts_cb(appl_layer_ctx_t *appl_ctx, zw_dsk_t *dsk, uint8_t status);
...
static void zwnet_exec_req_free(zwnet_exec_req_t *exec_req)
{
    ...
    {
        appl_node_info_sec2_t   *node_info_s2;
        appl_node_info_t        *node_info;

        node_info_s2 = (appl_node_info_sec2_t *)exec_req->user_dat;

        if (node_info_s2)
        {
            //Free allocated memory
            node_info = &node_info_s2->node_info;
...
@return     Zero on success; -1 if Supervision get command failed;
*/
...
#if 0   // *** Obsolete: Multi-channel encapsulation MUST be outside of multi-command encapsulation ***
...
l_CHECK_UNHANDLE_CMD:
    ...
    if (!cmd_handled && (cmd_len >= 2))
    {
        ...
        else if((sup_cmd_cls = zwnet_sup_cmd_cls_find(nw->sup_cmd_cls, cls, nw->sup_cmd_cls_cnt)) != NULL)
        {   //The command class is in the list of locally supported command class list
            if ((sup_cmd_cls->propty & BITMASK_CMD_CLS_SECURE) && ((sup_cmd_cls->propty & BITMASK_CMD_CLS_INSECURE) == 0))
            {   //Only accept secure message
                if ((msg_type & ZWNET_MSG_TYPE_SECURE) == 0)
                {
                    if (src_node != nw->ctl.nodeid)
...
//  }
    ...
    if ((rx_buf[0] == COMMAND_CLASS_SUPERVISION) && (rx_buf[1] == SUPERVISION_GET))
    {
        ...
        if (!zwif_find_cls(nw->ctl.ep.intf, COMMAND_CLASS_SUPERVISION))
        ...
        {   //Multi command encapsulated
            ...
            for (i=0; i < cmd_cnt; i++)
            {
                ...
                //Check whether there are enough bytes for the encapsulated command
                if (bytes_to_process >= enc_cmd_len)
                {   //Supervision get should be use for set Z-wave command
    ...
    if ((rx_buf[0] == COMMAND_CLASS_MULTI_CMD) && (rx_buf[1] == MULTI_CMD_ENCAP))
    {
        ...
        if (rx_len >= 3)
        {
            ...
            if (cmd_cnt > 0)
            {
                ...
                if (prm.spvsn_encap)
                {
                    zwnet_spvsn_rpt_send(&prm, (err_cnt == 0)? SPV_STATUS_SUCCESS : SPV_STATUS_FAIL);
                }
            }
        }
    }
    ...
    {   //Single command
        ...
        if (prm.spvsn_encap)
        {
            zwnet_spvsn_rpt_send(&prm, (res == 0)? SPV_STATUS_SUCCESS : SPV_STATUS_FAIL);
        }
    }
}
                ...
                //-------------------------------------------------
                    ...
                    if (!((nw->curr_op == ZWNET_OP_ADD_NODE || nw->curr_op == ZWNET_OP_RP_NODE) && nw->incl_on_behalf))
                    {
                        ...
                        if (nw->curr_op == ZWNET_OP_NONE)
                        {
                            ...
                            cb_prm.rpt_prm.node_info = *((appl_node_info_sec2_t *)exec_req->user_dat);
...
static int zwnet_4bytes_align_chk(void)
{
    ...
    for (i=0; i<2; i++)
    {
        ...
        wl_dsk_lst = malloc(sizeof(zw_wl_dsk_lst_t));
        if (wl_dsk_lst)
        {
            if (((uintptr_t)(wl_dsk_lst->dsk_lst)) & 0x03)
            {
                free(wl_dsk_lst);
                return 0;
            }
...
int zwnet_init(const zwnet_init_p init, zwnet_p *net)
{
    ...
    nw->appl_ctx.wl_sts_cb = zwnet_unsolic_wl_sts_cb;
...
static int zwnet_nif_set(zwnet_p  nw, uint8_t *cmd_cls_buf, uint8_t *buf_len)
{
    ...
    uint8_t     buf_index;
    uint8_t     buf_idx_secure;        //Buffer index for start of secure command class
...
int zwnet_load(zwnet_p nw, const char *node_info_file)
{
    ...
    if (node_info_file)
    {
        ...
        if (result < 0)
        {
            ALOGE("Load node info file '%s' failed: %d", node_info_file, result);
            return result;
        }
        ALOGI("Network info loaded ...");
    }
...
*/
void zip_send_DRL_cb(appl_layer_ctx_t *appl_ctx, int8_t tx_sts, void *user_prm, uint8_t node_id)
{
...
static void zwnet_node_rm_cb(appl_layer_ctx_t *appl_ctx, uint8_t sts, uint8_t rm_node_id)
{
    ...
    ALOGI("zwnet_node_rm_cb: status:%s, node id:%u",
                    (sts < 8)?rm_node_sts[sts]  : "unknown", (unsigned)rm_node_id);

    if (sts == REMOVE_NODE_STATUS_DONE || sts >= REMOVE_NODE_STATUS_FAILED)
    {
        zwnode_p    node;

        ALOGI("removed node id:%u", rm_node_id);
    ...
    else if (sts == REMOVE_NODE_STATUS_LEARN_READY)
    {
        ...
        if (nw->init.notify)
        {
            nw->init.notify(nw->init.user, ZWNET_OP_RM_NODE, OP_RM_NODE_LEARN_READY,NULL);
        }
    }
    else if (sts == REMOVE_NODE_STATUS_NODE_FOUND)
    {
        ...
        if (nw->init.notify)
        {
            nw->init.notify(nw->init.user, ZWNET_OP_RM_NODE, OP_RM_NODE_FOUND, NULL);
        }
    }
    else if (sts == REMOVE_NODE_STATUS_REMOVING_SLAVE
        || sts == REMOVE_NODE_STATUS_REMOVING_CONTROLLER)
    {
        //Notify the progress of the operation
        if (nw->init.notify)
        {
            nw->init.notify(nw->init.user, ZWNET_OP_RM_NODE, OP_RM_NODE_REMOVING, NULL);
        }
    }
}
...
*/
    ...
    {
        ALOGI("security 2: granted key mask:%02Xh, key exchange failed type:%u",
        ...
        || (add_ni->status == NODE_ADD_STATUS_SECURITY_FAILED))
    {
        ...
        ALOGI("new node id:%u", add_ni->node_id);
        ALOGI("listening=%u, optional functions=%u, sensor=%u", add_ni->listen, add_ni->optional, add_ni->sensor);
        ALOGI("Device type: basic=%02Xh, generic=%02Xh, specific=%02X", add_ni->basic, add_ni->gen, add_ni->spec);
        if (add_ni->cmd_cnt > 0)
        {
    ...
    else if ((add_ni->status == ADD_NODE_STATUS_ADDING_SLAVE)
        || (add_ni->status == ADD_NODE_STATUS_ADDING_CONTROLLER))
    {
        if (nw->init.notify)
        {
            nw->init.notify(nw->init.user, ZWNET_OP_ADD_NODE, OP_ADD_NODE_ADDING, NULL);
        }
    }
    else if (add_ni->status == ADD_NODE_STATUS_LEARN_READY)
    {
        ...
        if (nw->init.notify)
        {
            nw->init.notify(nw->init.user, ZWNET_OP_ADD_NODE, OP_ADD_NODE_LEARN_READY, NULL);
        }
    }
    else if (add_ni->status == ADD_NODE_STATUS_NODE_FOUND)
    {
        //Notify the progress of the operation
        if (nw->init.notify)
        {
            nw->init.notify(nw->init.user, ZWNET_OP_ADD_NODE, OP_ADD_NODE_FOUND, NULL);
        }
    }
    else
    {
...
@return
*/
static void zwnet_node_add_key_req_cb(appl_layer_ctx_t *appl_ctx, appl_node_keys_rpt_t req_keys_rpt)
{
    zwnet_p             nw = (zwnet_p)appl_ctx->data;
    void                *usr_param;
    add_node_sec2_fn    cb;
...
static void zwnet_node_add_rplc_hdlr(appl_layer_ctx_t *appl_ctx, appl_node_add_cb_prm_t *cb_prm)
{
    ...
    switch (cb_prm->rpt_type)
    {
        ...
        case NODE_ADD_KEYS_REPORT_V2:
            zwnet_node_add_key_req_cb(appl_ctx, cb_prm->rpt_prm.req_keys_rpt);
            break;
...
int zwnet_add_sec2_grant_key(zwnet_p net, uint8_t granted_keys, uint8_t grant_csa)
{
    ...
    res = zw_sec2_key_set(&net->appl_ctx, granted_keys, (net->csa_requested)? grant_csa : 0);
...
int zwnet_add(zwnet_p net, uint8_t add, sec2_add_prm_t *sec2_param, int incl_on_behalf)
{
    ...
    {
        if (!(net->ctl_cap & ZWNET_CTLR_CAP_INC_ON_BEHALF))
        {
    ...
    if (net->curr_op != ZWNET_OP_NONE)
    {
        ...
        {
            ALOGW("Current operation not completed yet, try again later");
            plt_mtx_ulck(net->mtx);
...
static void zwnet_failed_id_rm_cb(appl_layer_ctx_t *appl_ctx, uint8_t sts, uint8_t node_id)
{
    ...
    zwnet_notify(nw, ZWNET_OP_RM_FAILED_ID,
                    (sts == ZW_FAILED_NODE_REMOVED)? OP_DONE : OP_FAILED, NULL);
}
...
@return  0 on success, negative error number on failure
*/
static int zwnet_sm_ctlr_cc_ver_get(zwnet_p nw, zwif_p intf, uint16_t first_cls)
{
    int     result;
    zwifd_t ifd;
    uint8_t param;
        ...
        //----------------------------------------------------------------
            switch (evt)
            {
                ...
                case EVT_CTLR_TMOUT:
                    if (nw->sm_gen_retry++ < ZWNET_CTLR_SM_RETRY)
                    {
                        result = zip_node_ipv6_get(&nw->appl_ctx, zwnet_ctlr_id_addr_cb, nw->ctl.nodeid);
                        if (result == 0)
                        {
                            //Start timer
                            nw->sm_tmr_ctx = plt_tmr_start(&nw->plt_ctx, ZWNET_IP_NW_TMOUT, zwnet_sm_ctlr_tout_cb, nw);
                ...
                case EVT_CTLR_NODE_INFO:
                    {
                        ...
                        if (ver_intf)
                        {
                            ...
                            result = zwnet_sm_ctlr_cc_ver_get(nw, ver_intf, ver_intf->ep->intf->cls);
                            nw->sm_gen_retry = 0;
                ...
                case EVT_CTLR_NODE_INFO_RETRY:
                    {
                        ...
                        if (result == 0)
                        {
                            debug_zwapi_msg(&nw->plt_ctx, "Retrying to get Z/IP router controller node info ...");
...
/**
zwnet_unsolicited_node_list_cb - Unsolicited node list callback
@param[in]	appl_ctx            The application layer context
@param[in]	sts                 Status of previous call to "get node list"
@param[in]	node_list_ctlr_id   The node id of the controller which keeps the latest updated node list
@param[in]	node_list           Bitmask of nodes. The first bit in the bitmap represents node ID 1;
...
static void zwnet_unsolic_node_add_cb(appl_layer_ctx_t *appl_ctx, appl_node_add_cb_prm_t *cb_prm)
{
    ...
    if ((rpt_type == NODE_ADD_STATUS) || (rpt_type == NODE_ADD_DSK_REPORT_V2) || (rpt_type == NODE_ADD_KEYS_REPORT_V2))
    {
        plt_mtx_lck(nw->mtx);
        if (!((nw->curr_op == ZWNET_OP_ADD_NODE || nw->curr_op == ZWNET_OP_RP_NODE) && nw->incl_on_behalf))
        {
            ...
            if (nw->curr_op == ZWNET_OP_NONE)
            {
                nw->curr_op = ZWNET_OP_ADD_NODE;
                nw->incl_on_behalf = IOB_UNSOLICITED;
                debug_zwapi_msg(&nw->plt_ctx, "Enter into inclusion on-behalf or pre-kit mode ...");
            }
            else if (rpt_type == NODE_ADD_STATUS)
            {
                ...
                node_info_s2 = (appl_node_info_sec2_t *)malloc(sizeof(appl_node_info_sec2_t));
                if (!node_info_s2)
                {
                    return;
                }

                *node_info_s2 = cb_prm->rpt_prm.node_info;
                ...
                add_node_sts_req.node_id = node_info_s2->node_info.node_id;
                add_node_sts_req.action = EXEC_ACT_ADD_NODE_STS_CB;
                add_node_sts_req.user_dat = node_info_s2;
...
/**
zwnet_unsolic_wl_sts_cb - Unsolicited whitelist status callback function
@param[in]	appl_ctx    The application layer context
@param[in]	dsk         DSK
@param[in]	status	    status
@return
...
static void zwnet_unsolic_wl_sts_cb(appl_layer_ctx_t *appl_ctx, zw_dsk_t *dsk, uint8_t status)
{
    ...
    if (!dsk_str)
    {
        debug_zwapi_msg(&nw->plt_ctx, "zwnet_unsolic_wl_sts_cb: invalid dsk key");
        return;
    }

    debug_zwapi_msg(&nw->plt_ctx, "zwnet_unsolic_wl_sts_cb: status=%u, dsk key:%s", (unsigned)status, dsk_str);


    if (status == WL_STS_ADD_NODE_STARTED)
    {
        op_sts.type = ZWNET_STS_INFO_PREKIT_START;
...
/**
zwnet_wl_add - Add a whitelist entry
@param[in]	net		    Network
@param[in]	dsk		    Device Specific Key (DSK). The format of the DSK must be 8 groups of 5 digits seperated by '-' as shown in the example:
                        34028-23669-20938-46346-33746-07431-56821-14553
@param[in]	grant_keys	Restrict S2 grant keys to the device as specified, see SEC_KEY_BITMSK_XXX (optional).  Set to zero if unused.
...
@return		ZW_ERR_XXX
*/
int zwnet_wl_add(zwnet_p net, char *dsk, uint8_t grant_keys, wl_info_t *info, uint8_t info_cnt)
{
    uint8_t     dsk_bin[MAX_DSK_KEY_LEN];
    uint8_t     dsk_len;
    int         res;
    ...
    res = zw_wl_set(&net->appl_ctx, dsk_bin, dsk_len, grant_keys, info, info_cnt, 0);

    if (res < 0)
    {
        debug_zwapi_msg(&net->plt_ctx, "zwnet_wl_add error:%d", res);
...
/**
zwnet_wl_get_cb - Get a whitelist entry information report callback
@param[in]	appl_ctx    The application layer context
@param[in]	dsk         DSK
@param[in]	grant_keys	Restrict S2 grant keys to the device as specified, see SEC_KEY_BITMSK_XXX (optional). Zero if unused.
@param[in]	info		Buffer to store additional information of the device (optional).
@param[in]	info_cnt    Number of additional information stored in "info".
@return
*/
static void zwnet_wl_get_cb(appl_layer_ctx_t *appl_ctx, zw_dsk_t *dsk, uint8_t grant_keys, wl_info_t *info, uint8_t info_cnt)
{
    zwnet_p         nw = (zwnet_p)appl_ctx->data;
    char            *dsk_str;
    wl_info_fn      cb;
    ...
    cb = nw->wl_ctx.info_cb;
    usr_ctx = nw->wl_ctx.info_usr_ctx;
    ...
    if (!dsk_str)
    {
        debug_zwapi_msg(&nw->plt_ctx, "zwnet_wl_get_cb: invalid dsk key");
        return;
    }

    cb(usr_ctx, dsk_str, grant_keys, info, info_cnt);
    free(dsk_str);
...
/**
zwnet_wl_get - Get a whitelist entry information through callback
@param[in]	net		    Network
@param[in]	dsk		    Device Specific Key (DSK). The format of the DSK must be 8 groups of 5 digits seperated by '-' as shown in the example:
                        34028-23669-20938-46346-33746-07431-56821-14553
@param[in]	cb          Callback to report whitelist entry information
@param[in]	usr_ctx     User context used in callback
@return		ZW_ERR_XXX
*/
int zwnet_wl_get(zwnet_p net, char *dsk, wl_info_fn cb, void *usr_ctx)
{
    uint8_t     dsk_bin[MAX_DSK_KEY_LEN];
    uint8_t     dsk_len;
    int         res;
    ...
    net->wl_ctx.info_cb = cb;
    net->wl_ctx.info_usr_ctx = usr_ctx;

    res = zw_wl_get(&net->appl_ctx, dsk_bin, dsk_len, zwnet_wl_get_cb);

    if (res < 0)
    {
        debug_zwapi_msg(&net->plt_ctx, "zw_wl_get error:%d", res);
...
/**
zwnet_wl_clr - Clear a whitelist entry
@param[in]	net		    Network
@param[in]	dsk		    Device Specific Key (DSK). The format of the DSK must be 8 groups of 5 digits seperated by '-' as shown in the example:
                        34028-23669-20938-46346-33746-07431-56821-14553
@return		ZW_ERR_XXX
*/
int zwnet_wl_clr(zwnet_p net, char *dsk)
{
    uint8_t     dsk_bin[MAX_DSK_KEY_LEN];
    uint8_t     dsk_len;
    int         res;
    ...
    res = zw_wl_set(&net->appl_ctx, dsk_bin, dsk_len, 0, NULL, 0, WL_FLAG_CLR);

    if (res < 0)
    {
        debug_zwapi_msg(&net->plt_ctx, "zwnet_wl_clr error:%d", res);
...
/**
zwnet_wl_list_free - Free the structure wl_dsk_lst_t
@param[in]	dsk_lst		Pointer to structure to free
@return
*/
static void zwnet_wl_list_free(wl_dsk_lst_t *dsk_lst)
{
    uint8_t i;

    for (i=0; i<dsk_lst->dsk_cnt; i++)
    {
        free(dsk_lst->dsk_lst[i]);
    }
...
/**
zwnet_wl_list_get_cb - Get all whitelist entries report callback
@param[in]	appl_ctx    The application layer context
@param[in]	dsk_list    DSK list
@return
*/
static void zwnet_wl_list_get_cb(appl_layer_ctx_t *appl_ctx, zw_wl_dsk_lst_t *dsk_list)
{
    zwnet_p         nw = (zwnet_p)appl_ctx->data;
    char            *dsk_str;
    wl_list_fn      cb;
    void            *usr_ctx;
    wl_dsk_lst_t    *dsk_str_lst;
    int             i;
    //zwnet_cb_req_t  cb_req;

    plt_mtx_lck(nw->mtx);

    cb = nw->wl_ctx.list_cb;
    usr_ctx = nw->wl_ctx.list_usr_ctx;
    ...
    dsk_str_lst = (wl_dsk_lst_t *)calloc(1, sizeof(wl_dsk_lst_t) + dsk_list->dsk_cnt * sizeof(char *));
    ...
    dsk_str_lst->dsk_cnt = dsk_list->dsk_cnt;

    for (i=0; i<dsk_list->dsk_cnt; i++)
    {
        dsk_str = zwutil_dsk_to_str(dsk_list->dsk_lst[i].dsk, dsk_list->dsk_lst[i].dsk_len);

        if (!dsk_str)
        {
            debug_zwapi_msg(&nw->plt_ctx, "zwnet_wl_list_get_cb: invalid dsk key");
            zwnet_wl_list_free(dsk_str_lst);
            return;
        }
        dsk_str_lst->dsk_lst[i] = dsk_str;
    }

    cb(usr_ctx, dsk_str_lst);
    zwnet_wl_list_free(dsk_str_lst);
...
/**
zwnet_wl_list_get - Get all whitelist entries through callback
@param[in]	net		    Network
@param[in]	cb          Callback to report whitelist DSK list
@param[in]	usr_ctx     User context used in callback
@return		ZW_ERR_XXX
*/
int zwnet_wl_list_get(zwnet_p net, wl_list_fn cb, void *usr_ctx)
{
    int res;

    plt_mtx_lck(net->mtx);

    net->wl_ctx.list_cb = cb;
    net->wl_ctx.list_usr_ctx = usr_ctx;

    res = zw_wl_list_get(&net->appl_ctx, zwnet_wl_list_get_cb);

    if (res < 0)
    {
        debug_zwapi_msg(&net->plt_ctx, "zw_wl_list_get error:%d", res);
...
/**
zwnet_wl_list_clr - Clear all whitelist entries
@param[in]	net		network
@return		ZW_ERR_XXX
*/
int zwnet_wl_list_clr(zwnet_p net)
{
    int  res;

    res = zw_wl_set(&net->appl_ctx, NULL, 0, 0, NULL, 0, WL_FLAG_CLR_ALL);

    if (res < 0)
    {
        debug_zwapi_msg(&net->plt_ctx, "zwnet_wl_list_clr error:%d", res);
