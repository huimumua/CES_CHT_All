    ...
    WORD cmdLength) CC_REENTRANT_ARG
{
  ...
  switch (pCmd->ZW_Common.cmdClass)
  {
    ...
    case COMMAND_CLASS_CRC_16_ENCAP:
      ...
      {
        WRN_PRINTF("Security encapsulated CRC16 frame received.\n");
      }
  ...
  if((p->dnode != MyNodeID) && bridge_virtual_node_commandhandler(p, (BYTE*) pCmd, cmdLength)) {
    ...
    while(1)
    {
      ...
      if(ev == serial_line_event_message)
      {
        ...
        switch (((char*) data)[0])
        {
          /*case 'm':
          {
            static const uint8_t node_list[] = {6,7};
