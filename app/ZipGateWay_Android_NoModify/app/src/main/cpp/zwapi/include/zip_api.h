...
#define APPL_VERSION        7   ///<Application version
#define APPL_SUBVERSION     38  ///<Application sub-version
...
@{
*/
#define ZW_ERR_CACHE_AVAIL        2    /**< The data is available in the cache */
...
#define     OP_FW_UPLOADING             2   /**< Uploading firmware to device in progress */
...
@{
*/
#define ZWNET_STS_INFO_PREKIT_START 0   /**< Prekit add node protocol started (for ZWNET_OP_ADD_NODE->OP_ADD_NODE_PROTOCOL_START) */
...
typedef struct
{
    ...
    union
    {
        char		s2_dsk[MAX_DSK_STR_LEN + 1];     /**< S2 DSK (for ZWNET_STS_INFO_PREKIT_START) */
...
typedef struct config_rec
{
    ...
    int32_t     param_val;      /**< Configuration parameter value (signed integer)*/
} if_rec_config_t;
...
typedef void (*zwrep_switch_fn)(zwifd_p ifd, uint8_t on, time_t ts);
/**<
...
#define ZW_ALRM_EVT_CO_TEST				0x03    /**< Carbon monoxide test*/
...
#define ZW_ALRM_EVT_CO2_TEST			0x03    /**< Carbon dioxide test*/
...
#define ZW_ALRM_EVT_WATER_FLOW          0x06    /**< Water flow. Event parameter value: 1=no data, 2=below treshold, 3=above high treshold, 4=max*/
...
/**< 0x18 - 0x3F Reserved*/
#define ZW_ALRM_EVT_BARRIER_INIT					0x40    /**< Barrier performing Initialization process*/
...
#define ZW_ALRM_EVT_BARRIER_MECHANIC_EXCEED			0x43    /**< Barrier operation has exceeded physical mechanical limits. (For example: barrier has opened past the open limit)*/
...
#define ZW_ALRM_EVT_BARRIER_VACAT_MODE				0x47    /**< Barrier Vacation Mode*/
#define ZW_ALRM_EVT_BARRIER_BEAM_OBST				0x48    /**< Barrier Safety Beam Obstacle*/
#define ZW_ALRM_EVT_BARRIER_SNR_ERR					0x49    /**< Barrier Sensor Not Detected / Supervisory Error*/
#define ZW_ALRM_EVT_BARRIER_LOW_BATT				0x4A    /**< Barrier Sensor Low Battery Warning*/
...
#define ZW_ALRM_EVT_TIME_REMAINING		0x03    /**< Time remaining*/
...
#define ZW_ALRM_EVT_VOLATILE_ORGANIC_COMPOUND_LVL   0x06    /**< Volatile Organic Compound level*/

/** Z-wave alarm event for Siren*/
#define ZW_ALRM_EVT_SIREN_ACT						0x01    /**< Siren Active*/

/** Z-wave alarm event for Water Valve*/
#define ZW_ALRM_EVT_VALVE_OP						0x01    /**< Valve Operation*/
#define ZW_ALRM_EVT_MSTR_VALVE_OP					0x02    /**< Master Valve Operation*/
#define ZW_ALRM_EVT_SHORT_CCT					    0x03    /**< Valve Short Circuit*/
#define ZW_ALRM_EVT_MSTR_SHORT_CCT					0x04    /**< Master Valve Short Circuit*/
#define ZW_ALRM_EVT_CUR_ALRM				        0x05    /**< Valve Current Alarm*/
#define ZW_ALRM_EVT_MSTR_CUR_ALRM                   0x06    /**< Master Valve Current Alarm*/
...
#define ZW_ALRM_EVT_SCHED_START						0x01    /**< Schedule Started*/
#define ZW_ALRM_EVT_SCHED_FIN					    0x02    /**< Schedule Finished*/
#define ZW_ALRM_EVT_VLV_TBL_RUN_START			    0x03    /**< Valve Table Run Started*/
#define ZW_ALRM_EVT_VLV_TBL_RUN_FIN					0x04    /**< Valve Table Run Finished*/
...
@{
*/
#define ZW_ALRM_STS_DEACTIVATED         0    /**< Unsolicited alarm report is deactivated (push mode)*/
#define ZW_ALRM_STS_ACTIVATED           0xFF /**< Unsolicited alarm report is activated (push mode) or
...
typedef struct
{
	...
	uint8_t		ex_has_sequence;/**< Flag to indicate whether this report has sequence number.
									 1=valid; 0 = invalid(field ex_sequence_no does not have valid value).*/
...
typedef void (*zwrep_thrmo_md_fn)(zwifd_p ifd, uint8_t mode, time_t ts);
/**<
...
@param[in]	ifd	        interface
@param[out]	mode_buf    caller supplied buffer of size 255 bytes to store supported thermostat modes (ZW_THRMO_MD_XXX)
...
typedef void(*zwrep_csc_cfg_fn)(zwifd_p ifd, int slow_refresh);
/**<
...
/**
@}
@defgroup Prekit Prekit APIs
Support Prekit where whitelisted devices are added automatically to the Z-wave network without user interaction.
@ingroup zwarecapi
@{
*/
#ifndef WHITELIST_APPL
#define WL_INFO_TYPE_NAME       1   ///< Device name
#define WL_INFO_TYPE_LOC        2   ///< Device location

#define WL_MAX_NAME             63  ///< Maximum name length in bytes
#define WL_MAX_LOC              63  ///< Maximum location length in bytes

/** Pre-kit whitelist information */
typedef struct
{
	uint8_t     type;	                /**< Info type, WL_INFO_TYPE_XXX */
    union
    {
...
/** Pre-kit whitelist DSK list */
typedef struct
{
	uint8_t     dsk_cnt;	    /**< Number of DSK string in dsk_lst*/
    uint8_t     rsvd1;          /**< Padding to make sure dsk_lst is 4-byte aligned */
    uint16_t    rsvd2;          /**< Padding to make sure dsk_lst is 4-byte aligned */
    char        *dsk_lst[1];    /**< Place holder for DSK strings*/
} wl_dsk_lst_t;

typedef void (*wl_info_fn)(void *usr_ctx, char *dsk, uint8_t grant_keys, wl_info_t *info, uint8_t info_cnt);
/**<
callback to report whitelist entry information
@param[in]	usr_ctx     user context
...
typedef void (*wl_list_fn)(void *usr_ctx, wl_dsk_lst_t *dsk_list);
/**<
callback to report whitelist DSK list
@param[in]	usr_ctx     user context
...
/**<
Get a whitelist entry information through callback
@param[in]	net		    Network
@param[in]	dsk		    Device Specific Key (DSK). The format of the DSK must be 8 groups of 5 digits seperated by '-' as shown in the example:
                        34028-23669-20938-46346-33746-07431-56821-14553
...
/**<
Get all whitelist entries through callback
@param[in]	net		    Network
