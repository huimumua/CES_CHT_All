...
#define DTLS_SHUTDOWN_TIMEOUT_SEC   2       ///< DTLS wait for close notify timeout in seconds.
...
@param[out] pos_found   The position of the IP address found in the table (optional)
...
@param[in] ssl_type         SSL type, DTLS_SSL_TYPE_XXX
@param[in] send_alert       (Deprecated) Flag to indicate whether to send "Close Notify" alert to peer
@return
@pre    Caller must lock dtls_ctx->rdwr_mtx
*/
static void  dtls_ssl_close(ipaddr_ssl_t *ipaddr_ssl_ent, uint8_t ssl_type, int send_alert)
{
    int         ret;
                            ...
                            int rcx_packet_info, uint8_t *bound_addr, int *sock_fd)
{
    ...
    if (fd < 0)
    {
        debug_msg_show(tpt_ctx->plt_ctx, "Create udp socket failed");
        return 0;
    }
    ...
    if (reuse_addr)
    {
        ...
        {
            debug_msg_show(tpt_ctx->plt_ctx, "setsockopt SO_REUSEADDR failed");
            close(fd);
            return 0;
        }
    }
    ...
    {
        debug_msg_show(tpt_ctx->plt_ctx, "set fd O_NONBLOCK failed");
        close(fd);
        return 0;
    }
    ...
    if (res < 0)
    {
        debug_msg_show(tpt_ctx->plt_ctx, "dtls_sock_bind failed:%d", res);
        close(fd);
        return 0;
    }
...
{
    static const uint8_t    keep_alive_msg[] = {1, COMMAND_CLASS_ZIP, COMMAND_ZIP_KEEP_ALIVE, 0x80};
    int             i;
    int             k;
    int             bytes_sent;
    dtls_ctx_t      *dtls_ctx = (dtls_ctx_t *)tpt_ctx->dtls_ctx;
    ...
    while (1)
    {
        ...
        if (wr_req)
        {
            ...
            while (1)
            {
                ...
                ssl_type = DTLS_SSL_TYPE_SERVER;
                res = dtls_ip_ssl_get(tpt_ctx->dtls_ctx, dst_addr, dst_addr_len, ssl_type, &ssl, &pos);
                if (!((res == 0) && ssl))
                {   //Try using SSL type client
                    ssl_type = DTLS_SSL_TYPE_CLIENT;
                    res = dtls_ip_ssl_get(tpt_ctx->dtls_ctx, dst_addr, dst_addr_len, ssl_type, &ssl, &pos);
                }
                ...
                if ((res == 0) && ssl && (ssl_sock->state == DTLS_SSL_STA_DATA)
                    && !(send_pkt->flag & TPT_SEND_FLG_NEW_DTLS_SESS)   //Not using new DTLS session
                    && !((send_pkt->flag & TPT_SEND_FLG_USE_HOST_PORT) && (!ssl_sock->use_host_port)))
                {
                    //o.k. send the packet
                ...
                else
                {
                    if (res == 0)
                    {   //Session expired
                        dtls_ssl_close(ipaddr_ssl_ent, ssl_type, 1);
                    }
                    plt_mtx_ulck(dtls_ctx->rdwr_mtx);

                    //Create socket
                    if (send_pkt->flag & TPT_SEND_FLG_USE_HOST_PORT)
                    ...
                    //Add to "IP address - ssl" table
                    res = dtls_ip_ssl_add(dtls_ctx, ssl, dst_fd, dst_addr, dst_addr_len, DTLS_SSL_TYPE_CLIENT,
                    ...
                    //Reset send packet flag
                    send_pkt->flag &= ~TPT_SEND_FLG_NEW_DTLS_SESS;
                }
            }
...
void dtls_exit(tpt_layer_ctx_t *tpt_ctx)
{
    ...
    for (i=0; i<MAX_ZWAVE_NODE; i++)
    {
        if (tbl[i].ipaddr_sz)
        {
            dtls_ssl_close(&tbl[i], DTLS_SSL_TYPE_CLIENT, 1);
            dtls_ssl_close(&tbl[i], DTLS_SSL_TYPE_SERVER, 1);
        }
    }
