...
#define PROVISIONING_LIST_SIZE 232
#endif
 ...
 */
static pvs_result_t pvs_list_dev_import(FILE *strm, uint8_t ii)__attribute__((nonnull));
 ...
 *
 * \param item The provision to write to storage.
 * \return 0 on success, error code on failure.
 */
static pvs_result_t pvs_list_dev_store(FILE *strm, const struct provision *pvs, uint8_t ii) __attribute__((nonnull));
 ...
 */
static pvs_result_t pvs_list_dev_store(FILE *strm, const struct provision *pvs, uint8_t ii)
{
    size_t res;
    struct pvs_tlv *tlv;
...
static pvs_result_t pvs_list_dev_import(FILE *strm, uint8_t ii)
{
 ...
 * Do not modify iter.
 */
struct provision * provisioning_list_iter_get_index(provisioning_list_iter_t *iter, uint8_t idx)
{
    struct provision *pvs;
...
struct pvs_tlv * provisioning_list_tlv_dsk_get(uint8_t dsk_len, uint8_t *dsk, uint8_t type)
{
...
static void pvs_tlv_print(FILE *strm, struct pvs_tlv *tlv)
{
    int ii;
    while (tlv) {
