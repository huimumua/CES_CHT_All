...
@return
*/
static void zwnet_sm_csc_set_cb(appl_layer_ctx_t *appl_ctx, int8_t tx_sts, void *user_prm, uint8_t node_id)
{
    zwif_p  intf = (zwif_p)user_prm;
    zwnet_p nw = (zwnet_p)appl_ctx->data;
    uint8_t data[32];
...
@param[in]	skip_extra_cmd_cls      Flag to indicate whether to skip extra command classes added by Z/IP client to Z/IP gateway
                          ...
                          int tx_opt, int skip_extra_cmd_cls)
{
    ...
    if (intf)
    {
        ...
        if (nxt_intf)
        {
            ...
            result = zwif_get_report_ex(&ver_ifd, &param, 1,
                                     VERSION_COMMAND_CLASS_GET, zwif_tx_sts_cb, tx_opt);
...
static int zwnet_1_ep_sm_cls_hdlr(zwnet_p nw, uint8_t node_id, uint8_t ep_id, zwnet_1_ep_act_t act)
{
    ...
    static const zw_sm_hdlr_dat_t hdlr_dat[] = {
        ...
        {zwnet_sm_prot_sup_get, COMMAND_CLASS_PROTECTION, 0},
        ...
        //----------------------------------------------------------------
            {
                if (evt == EVT_S_EP_GRP_CNT_REPORT)
                {
                    ...
                    if (max_grp)
                    {
                        //Set SIS or primary controller node id into group 1 (Lifeline group)
                        if (nw->sm_job.auto_cfg)
                        {
                ...
                else if (evt == EVT_S_EP_GRP_1_MEMBER_REPORT)
                {
                    ...
                    //Check for nodelist controller id in the group
                    for (i=0; i<grp_cnt; i++)
                    {
                        if (grp_member[i].node_id == nw->nlist_ctl_id)
                        {
...
@return
*/
static void zwnet_mul_cmd_tx_sts_cb(appl_layer_ctx_t *appl_ctx, int8_t tx_sts, void *user_prm, uint8_t node_id)
{
    zwnet_p         nw = (zwnet_p)appl_ctx->data;
