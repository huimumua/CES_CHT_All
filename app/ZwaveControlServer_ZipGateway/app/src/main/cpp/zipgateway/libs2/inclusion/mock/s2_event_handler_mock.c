...
#define MOCK_CALL_COMPARE_INPUT_STRUCT_S2(P_MOCK, ARGUMENT, P_ACTUAL) do {                                       \
  MOCK_CALL_COMPARE_STRUCT_MEMBER_UINT8(P_MOCK, ARGUMENT, P_ACTUAL, zwave_event_t, evt.s2_event.peer.l_node);    \
  MOCK_CALL_COMPARE_STRUCT_MEMBER_UINT8(P_MOCK, ARGUMENT, P_ACTUAL, zwave_event_t, evt.s2_event.peer.r_node);    \
...
void s2_event_handler(zwave_event_t *p_actual_evt)
{
  ...
  if (p_mock->compare_rule_arg[0] == COMPARE_STRICT)
  {
    ...
    zwave_event_t * p_expect_evt = (zwave_event_t *)p_mock->expect_arg[0].pointer;
    ...
    switch (((zwave_event_t *)p_mock->expect_arg[0].pointer)->event_type)
    {
      case S2_NODE_INCLUSION_KEX_REPORT_EVENT:
        TEST_ASSERT_EQUAL_UINT8(p_expect_evt->evt.s2_event.s2_data.kex_report.security_keys,
                                p_actual_evt->evt.s2_event.s2_data.kex_report.security_keys);
      break;

      case S2_NODE_INCLUSION_PUBLIC_KEY_CHALLENGE_EVENT:
        TEST_ASSERT_EQUAL_UINT8_ARRAY(p_expect_evt->evt.s2_event.s2_data.challenge_req.public_key,
                                      p_actual_evt->evt.s2_event.s2_data.challenge_req.public_key,
                                      p_expect_evt->evt.s2_event.s2_data.challenge_req.length);
      break;

      case S2_NODE_INCLUSION_COMPLETE_EVENT:
        TEST_ASSERT_EQUAL_UINT8(p_expect_evt->evt.s2_event.s2_data.inclusion_complete.exchanged_keys,
                                p_actual_evt->evt.s2_event.s2_data.inclusion_complete.exchanged_keys);
      break;
