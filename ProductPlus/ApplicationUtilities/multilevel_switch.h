 ...
 * @param[in] endpoint is the destination endpoint
 */
void HandleSetCmd( BYTE bTargetlevel,
             BYTE bDuration,
             BYTE endpoint );
...
/**
 * @brief MultiLevelSwitchInit
 * Initialize the multilevel switch
 * @param[in] bEndPointCount number of multilevel switch endpoints
 * @param[in] pEndPointList pointer to a list of endpoints IDs
 */
void MultiLevelSwitchInit(BYTE bEndPointCount, BYTE* pEndPointList);
