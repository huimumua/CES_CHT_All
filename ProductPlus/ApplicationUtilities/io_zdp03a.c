...
void ZDP03A_InitHW(
  ...
  VOID_CALLBACKFUNC(pSetPowerDownTimeout)(BYTE)) /*function-pointer to battery ZCB_SetPowerDownTimeout(BYTE)*/
{
   ...
   * Initialize keys.
   */
  if (!KeyDriverInit(pSetPowerDownTimeout))
  {
    ZW_DEBUG_SEND_NL();
    ZW_DEBUG_SEND_STR("Couldn't initialize Key Driver :(");
  }
...
void SetPinIn( ZDP03A_KEY key, BOOL pullUp)
{
  ...
  {
    case ZDP03A_KEY_1:
      KeyDriverRegisterCallback(KEY01, key, BITFIELD_KEY_DOWN | BITFIELD_KEY_UP | BITFIELD_KEY_PRESS |BITFIELD_KEY_HOLD |BITFIELD_KEY_TRIPLE_PRESS,  ZCB_KeyAction);
      break;
    case ZDP03A_KEY_2:
