...
void
rd_failing_node_probe(BYTE txStatus, void* user)
{
  uint8_t node = user - (void*) 0;
  if (ndb[node - 1])
  {
    rd_set_failing(ndb[node - 1], txStatus != TRANSMIT_COMPLETE_OK);
  ...
  for (i = start_with_node; i < ZW_MAX_NODES - 1; i++)
  {
    ...
    if (limit < clock_seconds())
    {
      ...
      else if (n->mode == MODE_ALWAYSLISTENING && (n->state == STATUS_DONE))
      {
        ...
        {
          rd_failing_node_probe(TRANSMIT_COMPLETE_FAIL, (void*) 0 + n->nodeid);
        }
        return;
      }
    }
    ...
    WORD cmdLength, void* user)
{
      ...
      && n->state == STATUS_ENUMERATE_ENDPOINTS)
  {
    ...
    /*If the old endpoint count does not match free up all endpoints old endpoints and create some new ones */
    if (n->nEndpoints != n_end_points + 1)
    {
      ep = list_head(n->endpoints);
      ep = list_item_next(ep);
      for (; ep; ep = list_item_next(ep))
      {
        WRN_PRINTF("Freeing endpoint !\n");
    ...
    ZW_APPLICATION_TX_BUFFER *pCmd, WORD cmdLength, void* user)
{
              ...
              & 0x7F))
  {
    ...
    if (ep->endpoint_info)
    {
      ...
      DBG_PRINTF("State selected is %d", ep->state);
    }
    else
    {
...
rd_ep_probe_update(rd_ep_database_entry_t* ep)
{
  ...
  DBG_PRINTF("EP probe nd=%i ep =%d state=%s\n", ep->node->nodeid,
  ...
  switch (ep->state)
  {
      ...
      else
      {
        ...
        { /* ep->node->nodeid == MyNodeID */
          ...
          if (ZW_RequestNodeInfo(ep->node->nodeid, 0))
          {
            ...
            {
                ctimer_set(&nif_request_timer, 1250, node_info_request_timeout,
                ep);
            } else {
    ...
    case EP_STATE_PROBE_SEC2_C2_INFO:
      ...
      if( ep->endpoint_id >0 )
      {
        ...
        if(rd_ep_class_support(ep,COMMAND_CLASS_SECURITY_2)) {
        ...
        } else if(rd_ep_class_support(ep,COMMAND_CLASS_SECURITY)) {
          if (!ZW_SendRequest(&p, secure_commands_supported_get, sizeof(secure_commands_supported_get),
                  SECURITY_COMMANDS_SUPPORTED_REPORT, REQUEST_TIMEOUT, ep, rd_ep_secure_commands_get_callback))
          {
            goto fail_state;
          }
        }
        else {
          //This endpoint does not support security
          ep->state = EP_STATE_PROBE_ZWAVE_PLUS - 1;
          goto next_state;
        }
      }
...
void
rd_probe_next_state(BYTE txStatus, void* user)
{
  rd_node_database_entry_t* n = (rd_node_database_entry_t*) user;
...
rd_probe_resume()
{
  ...
  {
    if (ndb[i]
        && (ndb[i]->state != STATUS_DONE && ndb[i]->state != STATUS_PROBE_FAIL
            && ndb[i]->state != STATUS_FAILING))
    {
  ...
  if ((i == ZW_MAX_NODES) && (probe_lock==0))
  {
...
void
rd_register_new_node(uint8_t node, uint8_t adding)
{
  rd_node_database_entry_t *n;
  rd_ep_database_entry_t * ep;
  //char name[16];

  DBG_PRINTF(" nodeid=%d %d\n", node, adding);
  ...
  if (n)
  {
    ...
    {
      WRN_PRINTF("Node probe is already in progress.\n");
      return;
    }
  }
  ...
  n->wakeUp_interval = 70 * 60; //Default wakeup interval is 70 minutes
  ...
  if (node == MyNodeID)
  {
    if (get_net_scheme() > 0)
    {
      n->security_flags = NODE_FLAG_SECURITY0;
    }
  }
...
rd_ep_database_entry_t*
rd_ep_next(uint8_t node, rd_ep_database_entry_t* ep)
{
  uint8_t i;
...
static void
rd_set_ep_name_and_location(rd_ep_database_entry_t* ep, char* name,
...
void
rd_set_ep_name(rd_ep_database_entry_t* ep, char* name, u8_t size)
{
  rd_set_ep_name_and_location(ep, name, size, ep->endpoint_location,
      ep->endpoint_loc_len);
}

void
rd_set_ep_location(rd_ep_database_entry_t* ep, char* name, u8_t size)
{

  rd_set_ep_name_and_location(ep, ep->endpoint_name, ep->endpoint_name_len,
      name, size);
}
