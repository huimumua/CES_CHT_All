...
struct
{
  ...
  BYTE retry;
  int image_size;         /* Image size excl. struct zw_eeprom_img_hdr */
 ...
 */
static void
fwupdate_get_callback(BYTE txstatus, void* user)
{
...
static void
fwupdate_status_callback(BYTE txstatus, void* user)
{
...
static void
fw_send_done(BYTE status, void* user);
...
fw_send_done(BYTE status, void* user)
{
  ...
  if (fw_fsm.retry)
  {
    if (fw_fsm.first_report * fw_fsm.fragment_size >= fw_fsm.image_size + sizeof(struct zw_eeprom_img_hdr))
    {
...
send_fragment()
{
  ...
  int len = fw_fsm.image_size + sizeof(struct zw_eeprom_img_hdr) - offset;
  len = len > fw_fsm.fragment_size ? fw_fsm.fragment_size : len;

  if (offset + fw_fsm.fragment_size >= fw_fsm.image_size + sizeof(struct zw_eeprom_img_hdr))
  {
    f->properties1 |= 0x80; //Mark this as the last fragment
  }

  eeprom_read(EXT_FW_START_ADDR + offset, &f->data1, len);
  ...
  *(&f->data1 + len + 1) = (crc >> 0) & 0xff;
  ...
  DBG_PRINTF("Send fragment offset %x size %i\n", offset, len);
...
send_firmware_prepare_report(zwave_connection_t *c, u8_t status)
{
  ...
  if(INITIATE_FWUPDATE == status) {
    uint8_t* buf;
    uint16_t crc;
    int len = sizeof(struct zw_eeprom_img_hdr) + fw_fsm.image_size;

    buf = (uint8_t*) malloc(len);
    eeprom_read(EXT_FW_START_ADDR , buf, len);
    crc = ZW_CheckCrc16(CRC_INIT_VALUE, buf, len);
    free(buf);
 ...
 */
static void
restore_eeprom(uint32_t download_len)
{
  u8_t buf[64];
  uint8_t ret = 0;
  ...
  DBG_PRINTF("Size of file sent by ZIP client is : %d(0x%x)\n", download_len, download_len);
  while (fw_fsm.image_size < download_len)
  {
    if ((download_len - fw_fsm.image_size) > sizeof(buf))
    {
      len_to_write = sizeof(buf);
    }
    else
    {
      len_to_write = (download_len - fw_fsm.image_size);
    }

    DBG_PRINTF("Burning EEPROM address %04x\n", fw_fsm.image_size);
    eeprom_read(EXT_FW_START_ADDR + sizeof(struct zw_eeprom_img_hdr) + fw_fsm.image_size, buf, len_to_write);
    ret = nvm_restore(fw_fsm.image_size, buf, len_to_write, &length_written);
    fw_fsm.image_size += length_written;
    if (ret)
    {
      break;
    }
...
burn_eeprom(void)
{
  ...
  if (fw_fsm.image_size < 0xFFFF)
  {
    DBG_PRINTF("Burning EEPROM address %04x\n", fw_fsm.image_size);

    eeprom_read(EXT_FW_START_ADDR + sizeof(struct zw_eeprom_img_hdr) + fw_fsm.image_size, buf, sizeof(buf));
    MemoryPutBuffer(fw_fsm.image_size, buf, sizeof(buf), burn_eeprom);
    fw_fsm.image_size += sizeof(buf);
  }
  else
  {
    ZW_WatchDogEnable();
...
backup_eeprom(void *user)
{
  ...
  ret = nvm_backup(fw_fsm.image_size, buf, sizeof(buf), &length_read);

  eeprom_write(EXT_FW_START_ADDR + sizeof(struct zw_eeprom_img_hdr) + fw_fsm.image_size, buf, length_read);
  fw_fsm.image_size += length_read;
...
prepare_image(void* user)
{
  ...
  MemoryGetBuffer(fw_fsm.image_size, buf, blk_sz);
  eeprom_write(EXT_FW_START_ADDR + sizeof(struct zw_eeprom_img_hdr) + fw_fsm.image_size, buf, blk_sz);
  fw_fsm.image_size += blk_sz;
...
FwUpdate_Handler(BYTE* pData, WORD bDatalen)
{
  ...
  if (blockno == 1)
  {
    ...
    {
      struct zw_eeprom_img_hdr hdr;
      struct zw_eeprom_img_hdr* hdr2 = (struct zw_eeprom_img_hdr*) p;

      get_eeprom_img_hdr(&hdr);
      block_ok = memcmp(&hdr, hdr2, sizeof(struct zw_eeprom_img_hdr)) == 0;
    }
...
activate_image(uint16_t firmware_id, uint8_t firmware_target, uint16_t crc, uint8_t activation)
{
  ...
  if (desc.firmware_id == ZW_FW_EEPROM_ID && fw_fsm.state == FW_FSM_IDLE)
  {
    ...
    if (SupportsCommand_func(FUNC_ID_NVM_BACKUP_RESTORE))
    {
      restore_eeprom(desc.firmware_len);
    }
    else
    {
      burn_eeprom();
