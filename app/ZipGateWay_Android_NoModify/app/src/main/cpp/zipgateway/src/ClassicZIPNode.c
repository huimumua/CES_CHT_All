...
reset_ip_association_state(uint8_t bStatus)
{
  ...
  clear_ip_association_state();
  send_complete(bStatus);
  ctimer_stop(&nak_wait_timer);
}

void
reset_ip_association_state_ex(uint8_t bStatus, void* user) {
...
static void
NOP_Callback(BYTE bStatus, void* user)
{
  uint8_t node;
 ...
 */
  if(cur_send_ima && (flags0 & (ZIP_PACKET_FLAGS0_ACK_RES | ZIP_PACKET_FLAGS0_NACK_RES) ))
  {
    ...
    *p++ = 5;
    ...
    len += add_ext_header(ZIP_PKT_BUF,sizeof(uip_buf), INSTALLATION_MAINTENANCE_REPORT,ima_buffer, sizeof(ima_buffer) );
  }
 ...
 */
static void
unsolicited_senddata_callback(BYTE bStatus, void* user)
{
  ctimer_stop(&nak_wait_timer);
      ...
      && (bStatus != TRANSMIT_COMPLETE_OK && bStatus != TRANSMIT_COMPLETE_ERROR)))
  {
    ...
    {
      SendUDPStatus( bStatus == TRANSMIT_COMPLETE_OK ? ZIP_PACKET_FLAGS0_ACK_RES :ZIP_PACKET_FLAGS0_NACK_RES , cur_flags1,&zw);
    }
  }
 ...
 */
static void
proxy_forward_callback(BYTE bStatus, void* user)
{
...
void
classic_assoc_set_callback(unsigned char bStatus, void* user)
{
...
void
association_remove_callback(uint8_t bStatus, void* user)
{
...
encap_and_send_ip_assoc_remove(struct IP_association* ipa)
{
  ...
  if(!rd_node_exists(ipa->han_nodeid)) {
      association_remove_callback(TRANSMIT_COMPLETE_OK,0);
      return;
  }
...
temp_assoc_callback(struct IP_association *a)
{
  ...
  unsolicited_senddata_callback(TRANSMIT_COMPLETE_FAIL, 0);
}
...
handle_version(const uint8_t* payload,u8_t len, BOOL ack_req)
{
  ...
  if(payload[1] == VERSION_COMMAND_CLASS_GET) {
    ...
    if(the_version)
    {
      ...
      ZW_SendDataZIP(&zw,&f,sizeof(f),  (void (*)(BYTE, void* user)) send_complete);
      return TRUE;
    }
  }
  return FALSE;
...
handle_ip_association(zwave_connection_t* c,const u8_t* payload,u8_t len,BOOL was_dtls)
{
  ...
  if((get_queue_state() == QS_SENDING_FIRST) && (payload[1] != ASSOCIATION_GET))
  {
    ...
    send_complete(TRANSMIT_COMPLETE_NO_ACK);
    return TRUE;
  }
...
proxy_supervision_command_handler(zwave_connection_t* c, const uint8_t* payload, u8_t len, BOOL was_dtls, BOOL ack_req)
{
  ...
  if(f->cmd == SUPERVISION_GET) {
    if(proxy_command_handler(c,payload+4,f->encapsulatedCommandLength,was_dtls,ack_req)) {
      ...
      ZW_SendDataZIP(c,&rep,sizeof(rep),(void (*)(BYTE, void* user)) send_complete);
      return TRUE;
    }
  }
    ...
    case UIP_PROTO_UDP:
    {
      ...
      else if( UIP_UDP_BUF ->destport == UIP_HTONS(DTLS_PORT) )
      {
        ...
        if(!session_done_for_uipbuf() ) {
          ...
          send_complete(TRANSMIT_COMPLETE_OK); //If session is not yet established we are done processing the package.
...
static void send_unsolicited_callback(BYTE status, void* d) {
  ts_param_t p;
  //d=d;
    ...
    BYTE cmdLength, uip_ip6addr_t* destIP, u16_t port, BOOL bAck)
{
  ...
  if( uip_ipaddr_prefixcmp(&cfg.pan_prefix, &c.ripaddr, 64) ){
    ...
    /* If the remote addr is not in the pan, we use the gateways own IP as source*/
    ipOfNode(&c.lipaddr, MyNodeID);
  }
  c.ripaddr = *destIP;
