...
static int zwutl_tag_wr(FILE *file, zwtag_p tag)
{
    ...
    if (fwrite(tag->data, tag->len, 1, file) != 1)
    {
        return ZW_ERR_FILE_WRITE;
    }
...
static void zwutil_thrmo_setp_dat_upgd(zwif_p intf)
{
    ...
    intf->data_item_sz = sizeof(if_thrmo_setp_cap_t) + sizeof(zwsetp_range_t) * (sup_type_cnt - 1);
			...
			case ZW_SUBTAG_CCH_1LVL_ELE_SZ:
				{
					if(!intf->cch_data)
					{
						intf->cch_data = calloc(1, intf->cch_dat_item_sz * intf->cch_dat_cnt);
...
int zwutl_ni_save(zwnet_p nw, const char *ni_file)
{
    ...
    {   //----------------------------------------
        ...
        zwutl_subtag_wr8(ZW_SUBTAG_AGGR_EP_CNT, curr_node->num_of_aggr_ep, &subtag, &tag->len);
...
//      zwutl_subtag_wr8(ZW_SUBTAG_CRC16_CAP, curr_node->crc_cap, &subtag, &tag->len);
        ...
        zwutl_subtag_wr8(ZW_SUBTAG_SENSOR, curr_node->sensor, &subtag, &tag->len);
        zwutl_subtag_wr8(ZW_SUBTAG_CATEGORY, curr_node->category, &subtag, &tag->len);
        zwutl_subtag_wr8(ZW_SUBTAG_BASIC, curr_node->basic, &subtag, &tag->len);
        zwutl_subtag_wr8(ZW_SUBTAG_S2_KEY_VALID, curr_node->s2_keys_valid, &subtag, &tag->len);
        zwutl_subtag_wr8(ZW_SUBTAG_S2_GRNT_KEY, curr_node->s2_grnt_keys, &subtag, &tag->len);
        ...
        {
            zwutl_subtag_wr(ZW_SUBTAG_DSK_STR, strlen(curr_node->s2_dsk) + 1, (uint8_t *)curr_node->s2_dsk, &subtag, &tag->len);
        }

        zwutl_subtag_wr32(ZW_SUBTAG_WAKEUP_INTV, curr_node->wkup_intv, &subtag, &tag->len);
        ...
        while (curr_ep)
        {
            ...
            {
                zwutl_subtag_wr8(ZW_SUBTAG_AGGR_MEMBER_CNT, curr_ep->aggr_ep_cnt, &subtag, &tag->len);
                zwutl_subtag_wr(ZW_SUBTAG_AGGR_MEMBERS, curr_ep->aggr_ep_cnt, curr_ep->aggr_members, &subtag, &tag->len);
            }
            ...
            while (curr_intf)
            {
                ...
                zwutl_subtag_wr8(ZW_SUBTAG_VER, curr_intf->real_ver, &subtag, &tag->len);
                ...
                {
					zwutl_subtag_wr8(ZW_SUBTAG_CCH_B2LVL, curr_intf->b2lvlcch, &subtag, &tag->len);
                    zwutl_subtag_wr8(ZW_SUBTAG_CCH_DAT_CNT, curr_intf->cch_dat_cnt, &subtag, &tag->len);
                    zwutl_subtag_wr8(ZW_SUBTAG_CCH_DAT_ITEM_SZ, curr_intf->cch_dat_item_sz, &subtag, &tag->len);
                    if(curr_intf->b2lvlcch == 0)
					{
						zwutl_subtag_wr(ZW_SUBTAG_CCH_DATA, curr_intf->cch_dat_cnt * curr_intf->cch_dat_item_sz,
										(uint8_t *)curr_intf->cch_data, &subtag, &tag->len);
					}
					else
					{
						int i = 0;
						...
						for (i = 0; i < curr_intf->cch_dat_cnt; i++, p1lvl++)
						{
							...
							{
								zwutl_subtag_wr8(ZW_SUBTAG_CCH_1LVL_ELE_SZ, p1lvl->cch_ele_sz, &subtag, &tag->len);
								zwutl_subtag_wr(ZW_SUBTAG_CCH_DATA, p1lvl->cch_ele_sz,
										(uint8_t *)p1lvl->pcch_ele, &subtag, &tag->len);
							}
						}
					}
                }
