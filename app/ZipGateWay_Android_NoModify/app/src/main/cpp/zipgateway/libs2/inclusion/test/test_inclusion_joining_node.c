...
/** This is not a real test case but a test to ensure that the joining node
 *  code in inclusion is correctly working when building for a slave, that is
 *  when ZW_CONTROLLER is not defined.
 */
void test_slave_build()
{
#ifdef ZW_CONTROLLER
  TEST_FAIL_MESSAGE("ZW_CONTROLLER is defined when building joining node (ZW Slave) test cases.");
#else
...
void __test_kex_joining_node_state_machine_ex(uint8_t csa) {
  int test_flavour;
  ...
  //  p_mock->expect_arg[1].pointer = m_temp_key;
    ...
    s2_inclusion_joining_start(&s2_context, &s2_con,csa);
    ...
    s2_echo_kex_report_frame[2] = csa ? 0x3 : 0x1;
...
void test_kex_joining_node_state_machine_ssa() {
  __test_kex_joining_node_state_machine_ex(0);
}

void test_kex_joining_node_state_machine_csa() {
  __test_kex_joining_node_state_machine_ex(1);
}
...
void test_kex_joining_timer_handling() {
  ...
  // After non-secure inclusion of the node we expect to receive neighbor discovery complete within 10 seconds.
  ...
  // After neighbor discovery we expect to receing the KEX Get within 10 seconds.
  mock_call_expect(TO_STR(s2_inclusion_set_timeout), &p_mock);
  p_mock->compare_rule_arg[0] = COMPARE_ANY;
...
void test_kex_inclusion_init_errors()
{
  ...
  uint8_t valid_keys_bits = 0x87;    // Security 2, class 0, 1, 2 keys and Security 0 network key.
  uint8_t i;
...
void test_kex_inclusion_invalid_key_report_set() {
  ...
  for (i = 0; i < ELEM_COUNT(key_report_fail_test_vector); i++)
  {
    ...
    zwave_event_t  * p_expected_inc_req_event = (zwave_event_t *)m_test_mem_pool[1];
    p_expected_inc_req_event->event_type = S2_NODE_INCLUSION_PUBLIC_KEY_CHALLENGE_EVENT;
//    p_expected_inc_req_event->evt.s2_event.s2_data.challenge_req.security_keys = key_report_fail_test_vector[i][0];
    p_expected_inc_req_event->evt.s2_event.s2_data.challenge_req.length = sizeof(m_test_public_key_a);
    memcpy(p_expected_inc_req_event->evt.s2_event.s2_data.challenge_req.public_key, m_test_public_key_a, sizeof(m_test_public_key_a));
...
//  mock_call_use_as_stub(TO_STR(keystore_network_key_write));
  ...
  mock_call_expect(TO_STR(S2_network_key_update), &p_mock);
  p_mock->compare_rule_arg[0]   = COMPARE_ANY;
  p_mock->expect_arg[1].value   = 0; // Temp key is expected to be stored @0 index.
  ...
  mock_call_expect(TO_STR(S2_network_key_update), &p_mock);
  p_mock->compare_rule_arg[0]   = COMPARE_ANY;
  p_mock->expect_arg[1].value   = 1; // Network key is expected to be stored @1 index.
 ...
 *
 *  This failure should result in a KEX Fail - TBD to be transmitted to the peer.
 */
...
void helper_func_verify_idle_state(struct S2 *p_context, uint8_t scheme, uint8_t keys)
{
  ...
  s2_inclusion_post_event(p_context);
}
