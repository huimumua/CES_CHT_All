...
#define NM_OP_WL_LIST_GET               17  ///< Get all whitelist entries
#define NM_OP_WL_GET                    18  ///< Get a whitelist entry info

#define NM_OP_TOTAL                     19  ///< Total number of operations
///@}
...
typedef struct
{
    uint8_t   rpt_type;     ///< report types : NODE_ADD_STATUS, FAILED_NODE_REPLACE_STATUS, NODE_ADD_KEYS_REPORT_V2, or NODE_ADD_DSK_REPORT_V2
    union
    {
...
#define WL_FLAG_CLR             1   ///<Flag to indicate clear a whitelist entry as specified in the DSK
#define WL_FLAG_CLR_ALL         2   ///<Flag to indicate clear all whitelist entries

#ifndef WHITELIST_APPL
#define WHITELIST_APPL          //Define to prevent re-definition in zip_api.h
...
/** Whitelist status */
#define WL_STS_ADD_NODE_STARTED 1   ///< Add node process started
...
typedef struct
{
	uint8_t     dsk_cnt;	    /**< Number of DSK in dsk_lst*/
    ...
    zw_dsk_t    dsk_lst[1];     /**< Place holder for DSK*/
} zw_wl_dsk_lst_t;
...
typedef void (*zw_wl_list_fn)(struct _appl_layer_ctx *appl_ctx, zw_wl_dsk_lst_t *dsk_list);
/**<
...
typedef void (*zw_wl_info_fn)(struct _appl_layer_ctx *appl_ctx, zw_dsk_t *dsk, uint8_t grant_keys, wl_info_t *info, uint8_t info_cnt);
/**<
...
typedef void (*zw_wl_sts_fn)(struct _appl_layer_ctx *appl_ctx, zw_dsk_t *dsk, uint8_t status);
/**<
callback to report whitelist status
@param[in]	appl_ctx    context
@param[in]	dsk         DSK
@param[in]	status	    status, WL_STS_XXX
*/
...
typedef struct _appl_layer_ctx
{
    ...
    zw_wl_sts_fn            wl_sts_cb;      ///< Whitelist status callback
...
typedef struct
{
    ...
    uint8_t         *zip_router;  ///< Z/IP router (gateway) address
...
/**<
Get whitelist entry through callback
@param[in]	appl_ctx	Context
@param[in]	dsk		    Device Specific Key (DSK)
@param[in]	dsk_len		Device Specific Key length
