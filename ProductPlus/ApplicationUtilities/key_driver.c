...
#define KEY_HOLD_THRESHOLD          (10)  //100 msec called 10 times for battry control
// Thresholds in milliseconds
#define KEY_TRIPLE_PRESS_THRESHOLD  (30) // 300 mSec

#define KEY_DRIVER_PRESS_TIMEOUT (KEY_TRIPLE_PRESS_THRESHOLD + 10)/10 // 1100 mSec
#define KEY_DRIVER_HOLD_TIMEOUT (KEY_HOLD_THRESHOLD + 10)/10 // 1100 mSec
...
VOID_CALLBACKFUNC(pPowerDownCallback)(BYTE);
...
BOOL
KeyDriverInit(VOID_CALLBACKFUNC(pPowerDownCallbackLocal)(BYTE))
{
  memset((BYTE *)keyList, 0, (NUMBER_OF_KEYS * sizeof(KEY_LIST_T)));

  pPowerDownCallback = pPowerDownCallbackLocal;
...
void KeyDriverRegisterCallback(KEY_NAME_T keyName, BYTE keyPin, BYTE keyEvents, VOID_CALLBACKFUNC(pCallback)(KEY_NAME_T keyName, KEY_EVENT_T keyEvent, BYTE holdCount))
{
  ...
  if (NUMBER_OF_KEYS > keyName)
  {
    ...
    ZW_DEBUG_KEY_DRIVER_SEND_NUM(keyEvent);
    ZW_DEBUG_KEY_DRIVER_SEND_NUM(keyName);
    ZW_DEBUG_KEY_DRIVER_SEND_NUM(keyPin);
    ZW_DEBUG_KEY_DRIVER_SEND_NL();
...
PCB(ZCB_EdgeCall)(BYTE bLevelValue)
{
  ...
  if (0 == bLevelValue)
  {
    ...
    if (NON_NULL(pPowerDownCallback))
    {
      pPowerDownCallback(KEY_DRIVER_PRESS_TIMEOUT);
    }
  ...
  else
  {
    ...
    if (FALSE == fKeyHoldThresholdExceeded)
    {
      ...
      if (3 == pressCount)
      {
        ...
        ZW_DEBUG_KEY_DRIVER_SEND_STR("TRIPLE");

        //if (NON_NULL( keyList[whichButton].pEventCallbacks[KEY_TRIPLE_PRESS] ))
...
PCB(ZCB_KeyHeldTimerCallback)(void)
{
  if (NON_NULL( pPowerDownCallback ))
  {
    pPowerDownCallback(KEY_DRIVER_HOLD_TIMEOUT);
  }
