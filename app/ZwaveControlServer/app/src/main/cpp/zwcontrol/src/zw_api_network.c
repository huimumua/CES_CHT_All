...
//              }
    ...
    //Handle basic command class request
    else if (cmd_buf[0] == COMMAND_CLASS_SWITCH_ALL)
    {
        int         result;
        ...
        if (cmd_buf[1] == SWITCH_ALL_REPORT)
        {
            ...
            //Prepare the report
            c_buf[0] = COMMAND_CLASS_SWITCH_ALL;
            c_buf[1] = SWITCH_ALL_REPORT;
            c_buf[2] = cmd_buf[2];
            ...
            if (result < 0)
            {
                debug_zwapi_msg(&nw->plt_ctx, "Send Switch all rpt with error:%d", result);
            }
        }
    }
    /* Need to add this ?
                                              ...
                                              uint8_t rx_len, uint8_t *rx_buf)
{
    ...
    if (node)
    {
        node_sec_ena = (node->sec_incl_failed || nw->ctl.sec_incl_failed)? 0 : 1;
    }
...
int32_t zwnet_ctlr_info_get_result(zwnet_p nw, uint8_t is_create_nw, cJSON *jsonRoot)
{
    int             result;
    int             i;
    int             j;
    ...
    if (result == 0)
    {
        ...
        sprintf(str, "%08X", nw->homeid);

        cJSON_AddStringToObject(jsonRoot, "Home Id", str);
        cJSON_AddNumberToObject(jsonRoot, "Node Id", nw->ctl.nodeid);
    }
    else
    {
    ...
    if (result == 0)
    {
        ...
        cJSON_AddStringToObject(jsonRoot, "Basic Api Version", (char *) nw->ctl.basis_api_ver);
        cJSON_AddStringToObject(jsonRoot, "Lib Type", lib_types[nw->ctl_lib_type]);
    }
    else
    {
    ...
    if (result == 0)
    {
        ...
        sprintf(str, "%04X", serial_app_ver);
        cJSON_AddStringToObject(jsonRoot, "App Version", str);

        sprintf(str, "%04X", hci_cap.manf_id);
        cJSON_AddStringToObject(jsonRoot, "manufacturer id", str);

        sprintf(str, "%04X", hci_cap.manf_pdt_typ);
        cJSON_AddStringToObject(jsonRoot, "Manufacturer Product Type", str);

        sprintf(str, "%04X", hci_cap.manf_pdt_id);
        cJSON_AddStringToObject(jsonRoot, "Manufacturer Product Id", str);
    ...
    if (result == 0)
    {
        ...
        cJSON_AddNumberToObject(jsonRoot, "Serial API version", init_dat.version);
        cJSON_AddNumberToObject(jsonRoot, "Capabilities", init_dat.cap);

        sprintf(str, "%02X", init_dat.chip_typ);
        cJSON_AddStringToObject(jsonRoot, "Chip Type", str);

        sprintf(str, "%02X", init_dat.chip_ver);
        cJSON_AddStringToObject(jsonRoot, "Chip Verion", str);
        ...
        cJSON_AddItemToObject(jsonRoot, "NodeIDList", jsonArray);
        ...
        for (i = j = 0; i < 232; i++)
        {
            if (((init_dat.nodes[i>>3]) >> (i & 7)) & 0x01)
            {
                ...
                cJSON_AddNumberToObject(jsonArray, "NodeID", i + 1);
...
//                  }
        ...
        if (result == 0)
        {
            ...
            cJSON_AddNumberToObject(jsonRoot, "SUC Node Id", suc_node_id);
        }
        else
        {
            return (result == SESSION_ERROR_RESP_TMOUT)? ZW_ERR_NO_RESP : ZW_ERR_COMM;
...
*/
int zwnet_init(const zwnet_init_p init, zwnet_init_ret_p ret, cJSON *jsonRoot)
{
    ...
    result = zwnet_ctlr_info_get_result(nw, 1, jsonRoot);
                              ...
                              uint8_t node_id, appl_node_info_t *node_info)
{
    ...
    else if(sts == ADD_NODE_STATUS_DONE)
    {
        if (nw->init.notify)
        {
            nw->init.notify(nw->init.user, ZWNET_OP_ADD_NODE, OP_DONE);
        }
    }
}
...
void    zwnet_node_info_dump(zwnet_p net)
{
    ...
    while (zw_node)
    {
        ...
        plt_msg_show(&net->plt_ctx, "Node security inclusion status:%s", (zw_node->sec_incl_failed)?  "failed" : "unknown");
