...
/*HACK I've got this should be in ipv46_nat.h but this is not so simple */
extern void
ip4to6_addr(uip_ip6addr_t* dst, uip_ipaddr_t* src);
...
/**
 * Bitmask of awake nodes
 */
static uint8_t awake_nodes[32];
...
static void
mb_send_done(uint8_t status, void* user);
...
{
  static uint16_t n = 0;
  black_list_crc16[n] = crc16_data(&uip_buf[UIP_LLH_LEN], uip_len, 0xFFFF);
  n++;
  if (n >= MAX_MAILBOX_ENTRIES)
    n = 0;
}
...
check_fingerprint_uip_buf()
{
  uint16_t crc;
  uint16_t i;
  crc = crc16_data(&uip_buf[UIP_LLH_LEN], uip_len, 0xFFFF);
  for (i = 0; i < MAX_MAILBOX_ENTRIES; i++)
    {
      if (crc == black_list_crc16[i])
        return TRUE;
    }
  return FALSE;
}
  ...
  while (mb)
    {
      ...
      if (uip_ipaddr_cmp(&iph->destipaddr, ip))
        {
          if (is_zip_pkt(mb->data) && uip_is_addr_unspecified(&mb->proxy))
            {
              memcpy(UIP_IP_BUF, mb->data, UIP_IPUDPH_LEN + ZIP_HEADER_SIZE);
 ...
 */
static void
mb_udp_ping_reply(u8_t status, void* data)
{
...
{
  mailbox_t* m;
  uint8_t dnode = nodeOfIP(&UIP_IP_BUF->destipaddr);
  uip_ip6addr_t ip;
  u8_t waiting;
  ...
  /*Only do fingerprint check on frames which are not from comming from the mailbox */
  if (check_fingerprint_uip_buf() && (mb_state.state == MB_STATE_IDLE))
    {
      WRN_PRINTF("Mailbox finger print check fail!.\n");
      return FALSE;
    }
  add_fingerprint_uip_buf();
...
static void
mb_send_no_more_information(uint8_t node)
{
  ...
  ts_param_t p;
  ts_set_std(&p, node);
  BIT8_CLR(node - 1, awake_nodes);
  if(!ZW_SendDataAppl(&p, (BYTE*) &f, sizeof(f), mb_send_done, 0)) {
    mb_send_done(TRANSMIT_COMPLETE_FAIL,0);
  }
}

void
mb_wakeup_notify(uint8_t node)
{
  if (rd_get_node_state(node) == STATUS_DONE)
    {
      DBG_PRINTF("Node %i is awake\n", node);
      BIT8_SET(node - 1, awake_nodes);
    }
}
...
{
  return BIT8_TST(n - 1, awake_nodes);
}
...
mb_init()
{
  memb_init(&mb_memb);
  list_init(mb_list);
  memset(awake_nodes, 0, sizeof(awake_nodes));
...
mb_state_transition(mb_event_t event)
{
  ...
  case MB_STATE_IDLE:
    if (event == MB_EVNET_TIME_TO_PING)
    ...
    else if (event == MB_EVENT_POP)
      {
        ...
        if(!ClassicZIPNode_input(mb_state.node, mb_send_done_event, TRUE)) {
  ...
  case MB_STATE_SENDING:
    ...
    if(mb_state.last_entry) {
      ...
      if(ClassicZIPNode_input(mb_state.node, mb_send_done_event, TRUE)==0) {
        mb_state.state = MB_STATE_SEND_NO_MORE_INFO;
        mb_state_transition(MB_EVENT_TRANSITION);
      }
      uip_len = 0;
...
mb_is_busy(void)
{
  return !((mb_state.state == MB_STATE_IDLE) || (mb_state.state ==  MB_STATE_SEND_NO_MORE_INFO_DELAYED));
}
...
void mb_node_transmission_ok_event(u8_t node) {
...
REGISTER_HANDLER(
    mb_command_handler,
    0,
    COMMAND_CLASS_MAILBOX, MAILBOX_VERSION, SECURITY_SCHEME_UDP);
