...
/**
zwif_color_sw_rpt_split - split and callback color switch report handler
@param[in]	nw          Network
@param[in]	cb_req		Callback request
@return
*/
static void zwif_color_sw_rpt_split(zwnet_p nw, zwnet_cb_req_t  *cb_req)
{
    uint16_t    rpt_len;

    rpt_len = cb_req->param.sim_rpt.rpt_len - 2;//two bytes of command class and set command

    cb_req->param.sim_rpt.rpt_len = 4; //fix report length

    while (rpt_len >= 2)
    {   //Callback color switch report handler
        ...
        if (rpt_len >= 2)
        {
            memmove(cb_req->param.sim_rpt.rpt + 2, cb_req->param.sim_rpt.rpt + 4, rpt_len);
        }
    }
}
...
@return
*/
static void zwif_set_cb(appl_layer_ctx_t *appl_ctx, int8_t tx_sts, void *user_prm, uint8_t node_id)
{
    ...
    if ((tx_sts == ZWHCI_NO_ERROR) && (cb_id == nw->cch_set_ctx.id) && nw->cch_set_ctx.rpt_len)
    {
        ...
        if (cb_req.param.sim_rpt.rpt[0] == COMMAND_CLASS_SWITCH_COLOR
            && cb_req.param.sim_rpt.rpt[1] == SWITCH_COLOR_REPORT)
        {   //Split the report into multiple reports
            zwif_color_sw_rpt_split(nw, &cb_req);
        }
        else
        {
            util_list_add(nw->cb_mtx, &nw->cb_req_hd,
...
@param[in]	ifd	        interface
@param[in]	cmd_buf		set command and parameters
@param[in]	buf_len		length of cmd_buf in bytes
@param[in]	cb		    Optional post-set polling callback function.  NULL if no callback required.
@param[in]	usr_param   Optional user-defined parameter passed in callback.
@return		ZW_ERR_xxx
*/
static int zwif_supervision_get(zwifd_p ifd, uint8_t *cmd_buf, int buf_len, zw_postset_fn cb, void *usr_param)
{
    zwnet_p     nw = ifd->net;
    zwnode_p    node;
	zwif_p      intf;
    ...
    //Get session id
    intf = zwif_find_cls(node->ep.intf, COMMAND_CLASS_SUPERVISION);
    if (intf && intf->data_cnt)
    {
        ssn_id_ptr = (uint8_t *)intf->data;
        ssn_id = *ssn_id_ptr;
    ...
    result = zwspv_add(ifd, cmd_buf, buf_len, ssn_id, cb, usr_param);
...
int zwif_level_set(zwifd_p ifd, uint8_t v, uint8_t dur, zw_postset_fn cb, void *usr_param)
{
    ...
    {   //Try using supervision get to set and poll
        result = zwif_supervision_get(ifd, cmd, cmd_len, cb, usr_param);
        if (result == 0)
        {
    ...
    result = zwif_exec_ex(ifd, cmd, cmd_len, zwif_set_cb, (void *)((uintptr_t)cb_id), 0, NULL);
    ...
    if (result == 0)
    {
        ...
        if (res != 0)
        {
            ...
            //No post-set polling for this multi-level switch. Update cache when transmission is o.k.
            ...
            if (v != 0xFF)
            {
                ...
                nw->cch_set_ctx.rpt[0] = COMMAND_CLASS_SWITCH_MULTILEVEL;
                nw->cch_set_ctx.rpt[1] = SWITCH_MULTILEVEL_REPORT;
                nw->cch_set_ctx.rpt[2] = v;
                plt_mtx_ulck(nw->cch_set_ctx.mtx);
            }
        }
    }
...
int zwif_switch_set(zwifd_p ifd, uint8_t on)
{
    ...
    {   //Save the set value for updating cache in tx status callback
        ...
        nw->cch_set_ctx.id = cb_id;
        nw->cch_set_ctx.rpt_len = 3;
        nw->cch_set_ctx.rpt[0] = COMMAND_CLASS_SWITCH_BINARY;
        nw->cch_set_ctx.rpt[1] = SWITCH_BINARY_REPORT;
    ...
    for (i=0; i<intf_grp_data->valid_grp_cnt; i++)
    {
        if (intf_grp_data->grp_info[i])
        {
            ...
            {   //Found the group
                ...
                {
                    cmd_lst[j] = grp_info_ent->cmd_lst[j].cls;
                }
                *cmd_cnt = j;
...
int zwif_basic_set(zwifd_p ifd, uint8_t v)
{
    ...
    {   //Save the set value for updating cache in tx status callback
        ...
        nw->cch_set_ctx.id = cb_id;
        nw->cch_set_ctx.rpt_len = 3;
        nw->cch_set_ctx.rpt[0] = COMMAND_CLASS_BASIC;
        nw->cch_set_ctx.rpt[1] = BASIC_REPORT;
        nw->cch_set_ctx.rpt[2] = v;
        plt_mtx_ulck(nw->cch_set_ctx.mtx);
    }
...
int zwif_dlck_op_set(zwifd_p ifd, uint8_t mode, zw_postset_fn cb, void *usr_param)
{
    ...
    if (cb)
    {   //Try using supervision get to set and poll
        result = zwif_supervision_get(ifd, cmd, 3, cb, usr_param);
        if (result == 0)
        {
            return ZW_ERR_QUEUED;
        }
...
int zwif_thrmo_md_set(zwifd_p ifd, uint8_t mode)
{
    ...
    if (result == 0)
    {
        ...
        if ((cmd[2] != ZW_THRMO_MD_RESUME) && intf && intf->data_cnt)
        {   //Have supported mode cache
            thrmo_mode_dat = (if_thrmo_mode_data_t *)intf->data;

            if (thrmo_mode_dat)
            {
                if (memchr(thrmo_mode_dat->mode, cmd[2], thrmo_mode_dat->mode_len) != NULL)
                {   //Supported mode
                    write_cache = 1;
                }
            }
...
/**
zwif_csc_cfg_set - configure the use of optional node capabilities for scene notifications
@param[in]	ifd		        interface
...
int zwif_color_sw_set(zwifd_p ifd, uint8_t cnt, uint8_t* id, uint8_t* val, uint8_t dur)
{
    ...
    {   //Save the set value for updating cache in tx status callback
        ...
        nw->cch_set_ctx.rpt_len = 2 + (cnt * 2);
        nw->cch_set_ctx.rpt[0] = COMMAND_CLASS_SWITCH_COLOR;
        nw->cch_set_ctx.rpt[1] = SWITCH_COLOR_REPORT;
        memcpy(nw->cch_set_ctx.rpt + 2, cmd + 3, cnt * 2);
        plt_mtx_ulck(nw->cch_set_ctx.mtx);
    }
