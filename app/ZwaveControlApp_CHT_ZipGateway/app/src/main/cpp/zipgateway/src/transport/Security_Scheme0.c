...
static void noce_get_callback(uint8_t status,void* user) {
  sec_tx_session_t* s = (sec_tx_session_t*) user;
...
static void msg1_callback(uint8_t status,void* user) {
  sec_tx_session_t* s = (sec_tx_session_t*) user;
  s->tx_code = status;
...
static void msg2_callback(uint8_t status,void* user) {
  sec_tx_session_t* s = (sec_tx_session_t*) user;
  s->tx_code = status;
...
static void tx_session_state_set(sec_tx_session_t* s,tx_state_t state) {
  ...
  case TX_DONE:
    s->param.dnode = 0;
    ctimer_stop(&s->timer);
    if(s->callback) s->callback(TRANSMIT_COMPLETE_OK,s->user);
    break;
  case TX_FAIL:
    s->param.dnode= 0;
    ctimer_stop(&s->timer);
    if(s->callback) s->callback(s->tx_code,s->user);
    break;
  }
}
