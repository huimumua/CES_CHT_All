...
/*! @mainpage Z-Ware Portal C API
 *
 * @section intro_sec Introduction
 *
 * This library exposes C language API for Z-Ware Portal library. The portal serves as a central control unit (server) for controlling Z/IP gateways (clients) and their attached Z/IP or Z-wave slave devices.
 *
 *
 * @section install_sec Installation
 *
 * @subsection build Build:
 * Run these commands to build external OpenSSL library:
 *
 *      source install_openssl_lib.sh                  #only need to run once
 *
 *      source build_openssl_lib.sh linux [debug | release]  #build either debug or release library version
 *
 * Run this command to build Z-Ware Portal library:
 *
 *      make TARGET_PRODUCT=PORTAL_OPENSSL [DEBUG=1]   #build either debug or release library version
 *
