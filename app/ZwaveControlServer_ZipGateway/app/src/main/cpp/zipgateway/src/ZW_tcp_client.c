...
parse_portal_config(uint8_t *buf, uint8_t len) REENTRANT
{
  ...
  if (len >= config_len)
  {
    ...
    Ext_Eeprom_Config_Write(EXT_PORTAL_CONFIG_START_ADDR, 0, portal_config,
        sizeof(struct pushed_configuration));
...
zip_tcp_client_appcall(void *data)REENTRANT
{
  ...
  if (uip_connected())
  {
    ...
    if (gssl_client_ctx == NULL)
    {
      ...
      gssl_client_ctx = SSL_CTX_new(TLSv1_1_client_method());
#endif
      if (gssl_client_ctx == NULL)
      {
...
send_ipv6_to_tcp_ssl_client(void)
{
  ...
  if ((gisZIPRReady == ZIPR_READY) && (uip_packetqueue_free_len() > 1))
  {
    ...
    process_post_synch(&tcpip_ipv4_process, 0, tcp_tunnelconn);
  }
  else
  {
