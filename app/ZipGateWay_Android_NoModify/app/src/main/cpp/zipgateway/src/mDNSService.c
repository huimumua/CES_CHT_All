...
static char reply[UIP_UDP_APP_SIZE + 512]; // this buffer is used for building mDNS replies
...
static void gen_domain_name_for_ip6(uip_ip6addr_t *ipv6_address, char *s);
static void gen_domain_name_for_ip4(uint8_t nodeid,char* s);
/**
...
//	char buf[256];
  ...
  //	printf("%i %i\n",i,j);
//	printf("dest=%s\n", domain_name_to_string2(m, dest, buf));
//	printf("src=%s\n", domain_name_to_string2(m, src, buf));
...
//	printf("a=%s\n",domain_name_to_string2(m,a,buf));
//	printf("b=%s\n",domain_name_to_string2(m,b,buf));
...
//  DBG_PRINTF("Adding %s TXT record ttl =%i\n", domain_name_to_string(0,ep_service_name(ep)), ttl);
  ...
  mode = ((ep->node->mode & 0xFF00) >> 8) | ((ep->node->mode & 0x00FF) << 8);
  txt_add_binary(m, "mode", (char*) &(mode), sizeof(mode));
  txt_add_binary(m, "productID", (char*) &(ep->node->manufacturerID), 6);
  ...
  icon =
      ( ((ep->installer_iconID & 0xFF00) >> 8) | ((ep->installer_iconID & 0x00FF) << 8) )<<16 |
      ((ep->user_iconID & 0xFF00) >> 8) | ((ep->user_iconID & 0x00FF) << 8) ;
...
//  DBG_PRINTF("Adding AAAA record for %s\n", domain_name_to_string(0,lqn));
  ...
  h->rlength = UIP_HTONS(16); //length of ipv6 address
...
static int add_arpa6_for_node(rd_node_database_entry_t* node, dns_message_t* m)
{
  ...
  *s = 0;
  ...
  gen_domain_name_for_ip6(&node->ipv6_address, s);
...
void mdns_endpoint_nofify(rd_ep_database_entry_t* ep, u8_t express)
{
  ...
  if (express)
  {
    ...
    //DBG_PRINTF("mdns_endpoint_nofify\n");
...
static void send_answers(void* data)
{
  ...
  /**
   * This buffer must be large enough to hold a whole dns reply + plus 1 additional record.
                    ...
                    if (!(query_m.hdr->flags1 & 0x84)) { /* If the msg is nor response and server is not authority for domain. Probe denials have these flags */
                        ...
                        if (((qclass & 0x7F) > RR_CLASS_IN)) {
                            ...
                            if (qtype > RR_TYPE_A) {
                                ...
                                if (are_we_lex_later_ip6((uint8_t *)query_m.ptr, &nd->ipv6_address)) {
...
#endif
      gen_domain_name_for_ip6(&nd->ipv6_address, s);
 ...
 */
static int start_name_probe(void (*callback)(int bSucsess, void*), void* ctx)
{
...
{
  query_session_t* qs = &the_query_session;
  qs->rd_node_database_entry = ep;
  qs->prob_type = SERVICE;
  return start_name_probe(callback, ctx);
}

int mdns_node_name_probe(rd_node_database_entry_t* nd, void (*callback)(int bSucsess, void*), void*ctx)
{
  query_session_t* qs = &the_query_session;
  qs->rd_node_database_entry = (void* )nd;
  qs->prob_type = NODE;
  ...
  PROCESS_BEGIN()
    ...
    while (1)
    {
      ...
      } else if (ev == PROBE_DONE_EVENT)
      {
        ...
        {
          ERR_PRINTF("Name probing done and duplicates found\n");
        }
        cb = qs->callback;
