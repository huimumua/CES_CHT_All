        ...
        case SPV_STATUS_WORKING:
            {
                ...
                if (tmout < 0xFE)
                {
                    spv_ent->tmout = ((tmout - 0x79) * 60 * SPV_TICK_PER_SEC) + SPV_ADDTNL_TIMEOUT + spv_ctx->tmr_tick;
                    break;
                }
...
static void zwspv_add_poll(zwspv_ctx_t *spv_ctx, spv_ev_msg_add_t *add_msg)
{
    ...
    cmd_buf[3] = add_msg->buf_len;
    memcpy(cmd_buf + 4, add_msg->cmd_buf, add_msg->buf_len);

    if (zwif_exec_ex(&spv_ent->ifd, cmd_buf, add_msg->buf_len + 4, NULL, NULL, ZWIF_OPT_POLL, NULL) >= 0)
    {   //O.k.
...
@param[in]	ifd	        interface
@param[in]	cmd_buf		z-wave interface set command and parameters
@param[in]	buf_len		length of cmd_buf in bytes
@param[in]	ssn_id	    session id
@param[in]	cb		    post-set supervision callback function.
...
*/
int zwspv_add(zwifd_p ifd, uint8_t *cmd_buf, int buf_len, uint8_t ssn_id, zw_postset_fn cb, void *usr_param)
{
    zwspv_ctx_t   *spv_ctx = ifd->net->spv_ctx;
    spv_evt_msg_t msg;

    if (buf_len > SPV_MAX_CMD_LEN)
    {
    ...
    msg.ev_msg.add.buf_len = buf_len;
    msg.ev_msg.add.cb = cb;
    msg.ev_msg.add.usr_param = usr_param;
    memcpy(msg.ev_msg.add.cmd_buf, cmd_buf, buf_len);
