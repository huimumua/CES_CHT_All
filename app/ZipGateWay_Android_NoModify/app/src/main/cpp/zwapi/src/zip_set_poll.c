...
static void zwspoll_rpt_rcv_chk(zwspoll_ctx_t *spoll_ctx, spoll_ev_msg_rpt_t *rpt_msg)
{
    ...
    else if (rpt_msg->ifd.cls == COMMAND_CLASS_DOOR_LOCK)
    {
        ...
        {
            if ((spoll_ent->target != 0xFF) && (rpt_msg->value == 0))
            {   //Other target value means door unsecured is enough to meet the desired target
                target_hit = 1;
            }
        }
    }

    //Remove the entry if target is hit
    if (target_hit)
    {
        if (spoll_ent->cb)
        {
            zwspoll_cb(&spoll_ent->ifd, spoll_ent->cb, spoll_ent->usr_param, ZWPSET_REASON_TGT_HIT);
        }
...
*/
static int zwspoll_rpt_rcv(zwifd_p ifd, uint8_t value)
{
