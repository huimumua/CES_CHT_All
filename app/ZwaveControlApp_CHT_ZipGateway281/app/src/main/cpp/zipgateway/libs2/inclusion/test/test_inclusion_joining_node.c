...
#ifdef ZW_CONTROLLER
#define UNIT_TEST_TEMP_KEY_SECURE             3       //< Value identifying index for the temporary key in S2 network key when transmitting secure frames.
#define UNIT_TEST_NETWORK_KEY                 4       //< Value identifying index for the temporary key in S2 network key when transmitting secure frames.
#else
...
void test_kex_joining_node_state_machine_csa(void) {
  ...
  //  p_mock->expect_arg[1].pointer = m_temp_key;
    ...
    s2_context.inclusion_peer.class_id = 0xFF;
    ...
    s2_context.inclusion_peer.class_id = UNIT_TEST_TEMP_KEY_SECURE;
    s2_inclusion_post_event(&s2_context);
...
//  s2_context.buf    = s2_transfer_end_frame;
//  s2_context.length = sizeof(s2_transfer_end_frame);
//  s2_context.inclusion_peer.class_id = 0x00;
//  s2_inclusion_post_event(&s2_context);
...
void helper_func_kex_get_frame(struct S2 *p_context)
{
  ...
  p_context->inclusion_peer.class_id = 0xFF;
}
...
void helper_func_echo_kex_report_frame(struct S2 *p_context, uint8_t scheme, uint8_t curve, uint8_t keys)
{
  ...
  memcpy((uint8_t *)p_context->buf, s2_frame, sizeof(s2_frame));
  p_context->length = sizeof(s2_frame);
  p_context->inclusion_peer.class_id = UNIT_TEST_TEMP_KEY_SECURE;
}
