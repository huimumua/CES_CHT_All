/**
@file  nw_reset.c - Sample application to reset Z/IP gateway attached controller's Z-wave network.
...
static void hl_nw_tx_cb(void *user, uint8_t tx_sts)
{
    ...
    {
        //plt_msg_show("Higher level appl send data completed successfully");
    }
    else
    {
...
int main(int argc, char **argv)
{
    ...
    while (1)
    {
        ...
        else if (appl_ctx.rst_status == RESET_NW_STS_UNKNOWN)
        {
            switch (choice)
            {
                case '1':
            ...
            printf("\n(1) %s\n(x) Exit\n", (appl_ctx.rst_status == RESET_NW_STS_PROGRESS)?
                   "Abort reset network operation" : "Reset network");
            printf("Select your choice:\n");
        }
        else if (appl_ctx.rst_status == RESET_NW_STS_DONE)
        {
            if (choice == 'x')
            {
                 //Exit and clean up
        ...
        else    //RESET_NW_STS_PROGRESS
        {
            switch (choice)
            {
                case '1':
                    ...
                    if (result == 0)
                    {
                        printf("Reset network operation aborted.\n");
                        appl_ctx.rst_status = RESET_NW_STS_UNKNOWN;
                    }
                    else
                    {
                        printf("Reset network operation can't be aborted, error:%d\n", result);
                    }
                    break;
