/**
 * @file CommandClassMultiChan.c
 * @brief This module implements the Multi Channel command class.
 * @author Thomas Roll
...
static void
CmdClassMultiChannelEncapsulateCmd(
...
/**
 * @brief Extracts the content of a multichannel frame and call the application command handler.
...

{
  ...
  do /*BitAdress = 0 -> run only through once*/
  {
    ...
    {
      if(TRUE == TransportCmdClassSupported(((ZW_APPLICATION_TX_BUFFER*)&pCmd->encapFrame)->ZW_Common.cmdClass, rxOpt->securityKey))
      {
    ...
    {
      CMD_CLASS_LIST* pNonSec = GetEndpointcmdClassList(FALSE, rxOpt->destNode.endpoint);
      CMD_CLASS_LIST* pSec    = GetEndpointcmdClassList(TRUE, rxOpt->destNode.endpoint);
      if( (NULL != pNonSec) && (NULL != pSec))
      {
        if(TRUE == CmdClassSupported( rxOpt->securityKey,
            ((ZW_APPLICATION_TX_BUFFER*)&pCmd->encapFrame)->ZW_Common.cmdClass,
